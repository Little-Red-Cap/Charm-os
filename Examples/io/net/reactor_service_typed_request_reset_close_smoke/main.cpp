...
int main() {
    ...
    if (!winsock.ok) {
        std::fputs("reactor service typed request reset close winsock startup failed\n", stderr);
        return 1;
    }

    NativeSocket listener{::socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)};
    if (!listener.valid()) {
        std::fputs("reactor service typed request reset close native listen socket failed\n", stderr);
        return 2;
    }

    if (!set_nonblocking(listener.value)) {
        std::fputs("reactor service typed request reset close native listen nonblocking failed\n", stderr);
        return 3;
    }
    ...
    util::u16 port = 0;
    for (util::u16 candidate = 32920; candidate < 33020; ++candidate) {
        sockaddr_in addr{};
        ...
        if (::listen(listener.value, 2) != 0) {
            std::fputs("reactor service typed request reset close native listen failed\n", stderr);
            return 4;
        }
        port = candidate;
        break;
    ...
    if (port == 0) {
        std::fputs("reactor service typed request reset close native bind failed\n", stderr);
        return 5;
    }
    ...
    if (!connected) {
        std::fputs("reactor service typed request reset close charm client connect failed\n", stderr);
        return 6;
    }
    ...
    if (!accepted || accept_error != 0 || !server.valid()) {
        std::fputs("reactor service typed request reset close native accept failed\n", stderr);
        return 7;
    }
    ...
    if (!client_driver.start()) {
        std::fputs("reactor service typed request reset close driver start failed\n", stderr);
        return 8;
    }

    CloseRequest request{{'h', 'o', 'l', 'd'}};
    auto sent = client_session.send_request<CloseOp>(request,
                                                     0,
                                                     1000,
                                                     ...
                                                     &client_state);
    if (!sent) {
        std::fputs("reactor service typed request reset close send_request failed\n", stderr);
        return 9;
    }
    ...
    if (!got_wire || native_error != 0 || wire_filled != static_cast<int>(wire.size())) {
        std::fputs("reactor service typed request reset close native recv failed\n", stderr);
        return 10;
    }
    if (wire[0] != 0x00u
        || wire[1] != 0x08u
        ...
        || wire[8] != 'l'
        || wire[9] != 'd') {
        std::fputs("reactor service typed request reset close wire mismatch\n", stderr);
        return 11;
    }

    if (!set_abortive_close(server.value)) {
        std::fputs("reactor service typed request reset close linger setup failed\n", stderr);
        return 12;
    }
    server.close();
    ...
    auto retried = client_session.send_request<CloseOp>(request,
                                                        1,
                                                        1000,
                                                        &ClientState::on_response,
                                                        &ClientState::on_timeout,
    ...
    if (!done) {
        std::fputs("reactor service typed request reset close timeout\n", stderr);
        return 13;
    }
    if (client_state.response_count != 0) {
        std::fputs("reactor service typed request reset close unexpected response\n", stderr);
        return 14;
    }
    if (client_state.timeout_count != 0) {
        std::fputs("reactor service typed request reset close unexpected timeout\n", stderr);
        return 15;
    }
    if (retried || retried.error() != net::errc::closed) {
        std::fputs("reactor service typed request reset close retry not rejected\n", stderr);
        return 16;
    }
    if (client_session.has_pending() || client_session.pending_count() != 0) {
        std::fputs("reactor service typed request reset close pending not cleared\n", stderr);
        return 17;
    }
    if (client_session.last_error() != net::errc::closed) {
        std::fputs("reactor service typed request reset close session error mismatch\n", stderr);
        return 18;
    }
    if (!client_driver.closed() || client_driver.last_error() != net::errc::closed) {
        std::fputs("reactor service typed request reset close driver state mismatch\n", stderr);
        return 19;
    }

    std::puts("net reactor service typed request reset close smoke: ok");
    return 0;
}
