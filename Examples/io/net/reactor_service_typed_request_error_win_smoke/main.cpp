...
int main() {
    ...
    if (!winsock.ok) {
        std::fputs("reactor service typed request error win winsock startup failed\n", stderr);
        return 1;
    }

    NativeSocket listener{::socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)};
    if (!listener.valid()) {
        std::fputs("reactor service typed request error win native listen socket failed\n", stderr);
        return 2;
    }

    if (!set_nonblocking(listener.value)) {
        std::fputs("reactor service typed request error win native listen nonblocking failed\n", stderr);
        return 3;
    }
    ...
    for (util::u16 candidate = 33820; candidate < 33920; ++candidate) {
        ...
        if (::listen(listener.value, 2) != 0) {
            std::fputs("reactor service typed request error win native listen failed\n", stderr);
            return 4;
        }
        port = candidate;
        break;
    ...
    if (port == 0) {
        std::fputs("reactor service typed request error win native bind failed\n", stderr);
        return 5;
    }
    ...
    if (!accepted || accept_error != 0 || !server.valid()) {
        std::fputs("reactor service typed request error win native accept failed\n", stderr);
        return 7;
    }
    ...
    if (!client_driver.start()) {
        std::fputs("reactor service typed request error win driver start failed\n", stderr);
        return 8;
    }

    ErrorRequest request{{'h', 'o', 'l', 'd'}};
    auto sent = client_session.send_request<ErrorOp>(request,
                                                     0,
                                                     1000,
                                                     ...
                                                     &client_state);
    if (!sent) {
        std::fputs("reactor service typed request error win send_request failed\n", stderr);
        return 9;
    }
    ...
    if (!got_wire || native_error != 0 || wire_filled != static_cast<int>(wire.size())) {
        std::fputs("reactor service typed request error win native recv failed\n", stderr);
        return 10;
    }
    if (wire[0] != 0x00u
        || wire[1] != 0x08u
        ...
        || wire[8] != 'l'
        || wire[9] != 'd') {
        std::fputs("reactor service typed request error win wire mismatch\n", stderr);
        return 11;
    }
    if (!client_session.has_pending() || client_session.pending_count() != 1) {
        std::fputs("reactor service typed request error win pending not armed\n", stderr);
        return 12;
    }

    static constexpr util::u8 urgent[]{'!'};
    if (!send_all(server.value, urgent, sizeof(urgent), MSG_OOB)) {
        std::fputs("reactor service typed request error win native oob failed\n", stderr);
        return 13;
    }
    ...
    auto retried = client_session.send_request<ErrorOp>(request,
                                                        1,
                                                        1000,
                                                        &ClientState::on_response,
                                                        &ClientState::on_timeout,
    ...
    if (!done) {
        std::fputs("reactor service typed request error win timeout\n", stderr);
        return 14;
    }
    if (client_state.response_count != 0) {
        std::fputs("reactor service typed request error win unexpected response\n", stderr);
        return 15;
    }
    if (client_state.timeout_count != 0) {
        std::fputs("reactor service typed request error win unexpected timeout\n", stderr);
        return 16;
    }
    if (retried || retried.error() != net::errc::io) {
        std::fputs("reactor service typed request error win retry not rejected\n", stderr);
        return 17;
    }
    if (client_session.has_pending() || client_session.pending_count() != 0) {
        std::fputs("reactor service typed request error win pending not cleared\n", stderr);
        return 18;
    }
    if (client_session.last_error() != net::errc::io) {
        std::fputs("reactor service typed request error win session error mismatch\n", stderr);
        return 19;
    }
    if (client_driver.closed() || client_driver.last_error() != net::errc::io) {
        std::fputs("reactor service typed request error win driver state mismatch\n", stderr);
        return 20;
    }

    std::puts("net reactor service typed request error win smoke: ok");
    return 0;
}
