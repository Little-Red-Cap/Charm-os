...
function Show-LogTail {
    ...
    Write-Output "[armv7a-qemu-runtime-package] log tail:"
...
$outFile = Join-Path $PSScriptRoot "qemu-runtime-package.log"
$errFile = Join-Path $PSScriptRoot "qemu-runtime-package.err.log"
...
$expected = @(
    ...
    "ARMv7-A phase complete, stage=runtime-package",
    "ARMv7-A phase, stage=task-syscall-frame"
)
$missing = $expected | Where-Object { -not $log.Contains($_) }

$phaseMarker = "ARMv7-A phase, stage=runtime-package"
...
if (($packageLog -notmatch "ARMv7-A runtime package, leaf=yes, binding=yes, current=yes, trap=yes, call=yes, thread=yes, loop=yes, live=yes, derived=yes, export=yes, package=yes")) {
...
Write-Output "[ok] armv7a qemu runtime-package smoke detected"
