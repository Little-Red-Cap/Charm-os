param(
    ...
    [string]$OutputRoot = "cmake-build-system-compiler-front-page-entry-opening-testimony-landing-compare-smoke",
    [string]$PythonExe = "",
    [switch]$Clean
)
...
foreach ($requiredPath in @($openingTestimonyLandingSmokeScript, $compareScript, $validateScript)) {
...
try {
    ...
    $blockedLandingPath = Join-Path $openingTestimonyLandingRootPath "blocked-empty-explanation-landing\front-page.entry-opening-testimony.landing.summary.json"
    if (
        (-not $Clean) -and
        ...
        Write-Host "[FRONT-PAGE-ENTRY-OPENING-TESTIMONY-LANDING-COMPARE-SMOKE] landing_bootstrap=reuse-existing"
    } else {
        Invoke-ExternalTool `
    ...
    foreach ($case in $cases) {
        ...
        Invoke-ExternalTool `
            ...
            -FailureMessage ("front page entry opening testimony landing compare failed for case '{0}'" -f $case.Name)
        ...
        Invoke-ExternalTool `
            ...
            -FailureMessage ("front page entry opening testimony landing compare validation failed for case '{0}'" -f $case.Name)
        ...
        Write-Host (
            "[FRONT-PAGE-ENTRY-OPENING-TESTIMONY-LANDING-COMPARE-SMOKE] case={0} verdict={1} changed={2} judgment_changed={3} preview_changed={4} evidence_changed={5}" -f
            $case.Name,
...
Write-Host ("[FRONT-PAGE-ENTRY-OPENING-TESTIMONY-LANDING-COMPARE-SMOKE] output_root={0}" -f $outputRootPath)
