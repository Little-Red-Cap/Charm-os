...
from compare_system_compiler_front_page_entry_opener import build_compare_summary_model
from system_compiler_front_page_route_lib import load_json, normalize_path


COMPARE_SCHEMA_PATH = "schemas/system_compiler.front_page_entry_opener_compare.v0.schema.json"
...
def validate_references(summary: dict, errors: list[str]) -> None:
    ...
    for field in (
        "baseline_opener_summary_path",
        "candidate_opener_summary_path",
        "output_root",
    ...
    for index, provenance in enumerate(summary.get("opener_provenance", [])):
        if not isinstance(provenance, dict):
            errors.append(f"opener_provenance[{index}]: invalid provenance entry")
            continue
        ensure_exists(provenance.get("source_summary_path"), f"opener_provenance[{index}].source_summary_path", errors)
        ensure_exists(
            provenance.get("source_report_markdown_path"),
            f"opener_provenance[{index}].source_report_markdown_path",
            errors,
        )
        ensure_exists(
            provenance.get("source_check_text_path"),
            f"opener_provenance[{index}].source_check_text_path",
            errors,
        )


def main() -> int:
    parser = argparse.ArgumentParser(description="Validate system compiler front_page entry opener compare summary.")
    parser.add_argument(
        "--summary",
        default="",
        help="Path to opener compare summary JSON. If omitted, --bundle-root/front-page.entry-opener.compare.summary.json is used.",
    )
    parser.add_argument(
        "--bundle-root",
        default="",
        help="Bundle root containing front-page.entry-opener.compare.summary.json.",
    )
    ...
    if args.summary:
        ...
        bundle_root = Path(args.bundle_root or "out/system-compiler-front-page-entry-opener-compare").resolve()
        summary_path = bundle_root / "front-page.entry-opener.compare.summary.json"
    ...
    try:
        ...
        artifact_context = summary.get("artifact_context", {})
        baseline_opener_path = Path(artifact_context.get("baseline_opener_summary_path", "")).resolve()
        candidate_opener_path = Path(artifact_context.get("candidate_opener_summary_path", "")).resolve()
        ...
        jsonschema.validate(load_json(baseline_opener_path), opener_schema)
        jsonschema.validate(load_json(candidate_opener_path), opener_schema)

        expected_summary = build_compare_summary_model(
            baseline_opener_path=baseline_opener_path,
            candidate_opener_path=candidate_opener_path,
        ...
        for field in (
            ...
            "change_summary",
            "field_changes",
            "opener_regression_surface",
            "opener_improvement_surface",
            "questions",
            "violations",
        ):
    ...
    print(f"[OK] opener verdict -> {summary.get('opener_verdict', '')}")
