param(
    ...
    [string]$OutputRoot = "out/net-lab-route-table-mutation-smoke",
    [int]$Jobs = 0,
    [switch]$Fresh
)
...
$targetName = "net-lab-route-table-mutation-smoke"
$exampleRoot = Join-Path $repoRoot "Examples\io\net\net_lab_route_table_mutation_smoke"
$buildDir = Join-Path $repoRoot "cmake-build-verify-net-lab-route-table-mutation-smoke"
...
try {
    Invoke-LoggedExternalTool `
        ...
        -FailureMessage "net lab route table mutation configure failed" `
        -Phase "configure"
    ...
    Invoke-LoggedExternalTool `
        ...
        -FailureMessage "net lab route table mutation build failed" `
        -Phase "build"
    ...
    $runOutput = Invoke-LoggedProgram `
        -Executable $exePath `
        -LogPath $runLogPath `
        -FailureMessage "net lab route table mutation run failed"
} finally {
    Pop-Location
}

$expectedLine = "net lab route table mutation smoke: ok"
if (($runOutput -join "`n") -notmatch [regex]::Escape($expectedLine)) {
    throw "net lab route table mutation smoke did not report success"
}

$checkLines = @(
    ("summary: {0}" -f $checkTextPath),
    "profile: net-lab-route-table-mutation-smoke",
