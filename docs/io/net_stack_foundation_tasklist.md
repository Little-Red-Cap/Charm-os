...
- `net.protocol.diagnostic_udp` 已把最小 UDP 诊断协议的服务端 / 客户端一起挂到 `UdpStackPump` 数据面上，`net_udp_diag_smoke` 可覆盖服务端 `ping / count / slow_count / meta` 回复以及 `unsupported / bad_request` 错误回复，`net_udp_diag_client_smoke` 可覆盖客户端请求发送、响应分发、错误回复、超时与取消后 stray reply 丢弃，`net_udp_diag_roundtrip_smoke` 可覆盖双端 `client -> ARP -> server -> reply` 真实往返闭环，`net_lab_udp_diag_forward_smoke` 则进一步把这条应用层 UDP 往返闭环推进到 `client -> router1 -> router2 -> server -> reply` 的自研 forwarding 数据面上，覆盖 proxy ARP、跨子网转发与 unsupported error reply 的真实穿越路径，`net_lab_udp_diag_forward_failure_smoke` 则把 routed timeout、cancel request 与 late/stray reply drop 这组失败路径也钉到同一条 forwarding 数据面上，`net_lab_udp_diag_forward_late_reply_smoke` 则进一步补齐“真实服务端已回复、跨路由最后一跳延迟把 reply 拖过 deadline、客户端 timeout 后再丢弃 late reply、随后恢复继续成功往返”这组更贴近真实链路抖动的语义，`net_lab_udp_diag_forward_route_churn_smoke` 则继续补上“请求已送达 server、reply 正在返程、router2 热删除 client route 后转为 ICMP destination unreachable 回 server，而 client 只观察到 timeout；恢复路由后下一次往返重新成功”这组 in-flight route churn 语义，`net_lab_udp_diag_forward_explain_smoke` 则继续把“client 通过 routed diagnostic UDP 远程查询 router1 的 forwarding explain 快照，并覆盖 explicit route、connected prefix、local drop、limited broadcast drop、TTL exhausted、no-route unreachable 与动态 add/remove direct route 后的 explain 切换”这组 explain-over-diag 语义压到同一条 UDP forwarding 数据面上，`net_lab_udp_diag_forward_inspect_smoke` 则进一步把“client 通过 routed diagnostic UDP 远程查询 router1 对目标地址的 forwarding inspect 结果，并同时拿到 explain 快照、当前 route_count，以及命中的显式 route 索引/条目，从而把 host-route -> bad /24 -> default route 的赢家切换、connected prefix 非表项命中与 no-route 回退”这组 inspect-over-diag 语义压到同一条 UDP forwarding 数据面上，`net_lab_udp_diag_forward_default_precedence_smoke` 则把“default route -> 坏的 /24 -> 正确的 /32 -> 删除 /32 回落到 /24 -> 再删除 /24 回落到 default route”这整条 longest-prefix precedence/fallback 链接到 UDP 诊断 forwarding 数据面上，`net_lab_udp_diag_forward_metric_smoke` 则继续把“同 /24 候选先按 lower metric 选路、同 metric 保持先插入者胜出、但更长 /32 host-route 仍压过 /24，再由更低 metric 的 /32 抢回”这整条 metric/host-route 优先级链压到同一条 UDP forwarding 数据面上，`net_lab_udp_diag_forward_route_table_mutation_smoke` 则继续把“default route overwrite、specific route overwrite、clear_routes 后 route miss 触发 unreachable 但客户端仍只观察到 timeout、随后 re-add default route 恢复真实往返”这整条 route table mutation 语义压到同一条 UDP forwarding 数据面上，`net_lab_udp_diag_forward_route_introspection_smoke` 则继续把“canonical route snapshot、default 删除后的 table 压缩、bad /24 与 host /32 的决策观测、clear 后 server 无决策但 connected prefix 观测仍保留”这组 introspection 语义压到同一条 UDP forwarding 数据面上，`net_lab_udp_diag_forward_precise_delete_smoke` 则再把“同 prefix 下先由更低 metric 候选接管、随后按 index 精确删除当前赢家并回退到剩余候选 route”这组 precise delete 语义压到同一条 UDP forwarding 数据面上，`net_lab_udp_diag_forward_proxy_arp_churn_smoke` 则再补上“新目标 IP 上 proxy ARP 会随 route presence 动态出现/消失，且恢复显式 route 后真实 server 往返重新成功”这组 ARP 与 forwarding 叠加语义
