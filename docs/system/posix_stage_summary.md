...
- newlib `_open()` now also has direct bridge smoke for flag translation: `O_NONBLOCK` remains observable through `fcntl(F_GETFL)`, `O_APPEND` appends at end-of-file on regular files, and `O_RDWR` yields a read/write descriptor that stays readable and writable through the same fd
