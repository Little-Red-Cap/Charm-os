...
int main() {
    ...
    if (!winsock.ok) {
        std::fputs("reactor service close win winsock startup failed\n", stderr);
        return 1;
    }
    ...
    if (port == 0) {
        std::fputs("reactor service close win listen failed\n", stderr);
        return 2;
    }

    NativeSocket client{::socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)};
    if (!client.valid()) {
        std::fputs("reactor service close win native socket failed\n", stderr);
        return 3;
    }
    ...
    if (::connect(client.value,
        ...
        std::fputs("reactor service close win native connect failed\n", stderr);
        return 4;
    }
    ...
    if (!accepted || accept_error != net::errc::ok) {
        std::fputs("reactor service close win accept failed\n", stderr);
        return 5;
    }
    ...
    auto route = server_session.set_deferred_route(0x72u,
                                                   ...
                                                   &server_state);
    if (!route) {
        std::fputs("reactor service close win route registration failed\n", stderr);
        return 6;
    }

    Driver server_driver{reactor, poller, server_binding, server_session};
    if (!server_driver.start()) {
        std::fputs("reactor service close win driver start failed\n", stderr);
        return 7;
    }
    ...
    if (!send_all(client.value, wire, sizeof(wire))) {
        std::fputs("reactor service close win native send failed\n", stderr);
        return 8;
    }
    client.close();
    ...
    if (!done) {
        std::fputs("reactor service close win timeout\n", stderr);
        return 9;
    }
    if (!server_state.late_reply_rejected) {
        std::fputs("reactor service close win late reply not rejected\n", stderr);
        return 10;
    }
    if (!server_driver.closed() || server_driver.last_error() != net::errc::closed) {
        std::fputs("reactor service close win driver state mismatch\n", stderr);
        return 11;
    }
    if (server_session.has_deferred() || server_session.deferred_count() != 0) {
        std::fputs("reactor service close win deferred not cleared\n", stderr);
        return 12;
    }
    if (server_session.last_error() != net::errc::closed) {
        std::fputs("reactor service close win session error mismatch\n", stderr);
        return 13;
    }

    std::puts("net reactor service close win smoke: ok");
    return 0;
}
