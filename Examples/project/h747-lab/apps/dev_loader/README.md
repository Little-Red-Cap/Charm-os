...
- Run the same evidence bundle and append the off-board QEMU ELF virtual-board smoke:
  ...
  `qemu_elf_capacity`, `qemu_elf_coverage`, and `qemu_elf_player_min_gui`
  tokens so the archived evidence shows the virtual runtime domain, capability
  contract, ELF load memory boundary, Store media, fixed display mode,
  frame/input/storage evidence counts, near-limit and over-limit ELF capacity,
