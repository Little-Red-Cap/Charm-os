param(
    ...
    [string]$OutputRoot = "out/net-lab-smoke",
    [int]$Jobs = 0,
    [switch]$Fresh
)
...
try {
    Invoke-LoggedExternalTool `
        ...
        -FailureMessage "net lab configure failed" `
        -Phase "configure"
    ...
    Invoke-LoggedExternalTool `
        ...
        -FailureMessage "net lab build failed" `
        -Phase "build"
    ...
    $runOutput = Invoke-LoggedProgram `
        -Executable $exePath `
        -LogPath $runLogPath `
        -FailureMessage "net lab run failed"
} finally {
    Pop-Location
}

$expectedLine = "net lab smoke: ok"
if (($runOutput -join "`n") -notmatch [regex]::Escape($expectedLine)) {
    throw "net lab smoke did not report success"
}

$checkLines = @(
    ("summary: {0}" -f $checkTextPath),
    "profile: net-lab-smoke",
