...
| Font | `player.font_resource` owns the app-common font input helpers, `FontResourceConfig` carries explicit font source kind plus sizes, host preview composes file-font config through the same helpers, and board ports can pass package bytes through `PlayerFontPackageResourceView`. | `CHARM_PLAYER_HOST_FILE_FONTS`, `CHARM_PLAYER_PC_FONT_CACHE`; profile log prints `host_file_fonts`. | FreeType/VFS font buffers, Win32/GDI glyph cache vectors, preview argv strings. | Define the concrete board font package binary/layout behind `FontResourceKind::Package`. |
...
- `PlayerFontPackageResourceView` gives board code a named place to pass package bytes/key/size metadata without pretending file paths exist, and `player.font_resource` provides the common config builders for builtin/file/package modes.
