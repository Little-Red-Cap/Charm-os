...
- `net.protocol.diagnostic_udp` 已把最小 UDP 诊断协议的服务端 / 客户端一起挂到 `UdpStackPump` 数据面上，`net_udp_diag_smoke` 可覆盖服务端 `ping / count / meta` 回复以及 `unsupported / bad_request` 错误回复，`net_udp_diag_client_smoke` 可覆盖客户端请求发送、响应分发、错误回复、超时与取消后 stray reply 丢弃
