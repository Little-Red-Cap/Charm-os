...
namespace {
    ...
    [[nodiscard]] bool service_step(HostNode& client_node,
                                    ForwardHop& router1,
                                    ForwardHop& router2,
        ...
        if (!client_progress) {
            std::fprintf(stderr,
                         "udp diag forward client service error=%d\n",
        ...
        if (!router1_progress) {
            std::fprintf(stderr,
                         "udp diag forward router1 service error=%d\n",
        ...
        if (!router2_progress) {
            std::fprintf(stderr,
                         "udp diag forward router2 service error=%d\n",
        ...
        if (!server_progress) {
            std::fprintf(stderr,
                         "udp diag forward server service error=%d\n",
...
int main() {
    ...
    if (!client_init || !client_node.ready()) {
        return fail("net lab udp diag forward smoke client init failed\n", 1);
    }
    ...
    if (!router1_a_init
        ...
        || !router1.ready()) {
        return fail("net lab udp diag forward smoke router1 init failed\n", 2);
    }
    ...
    if (!router2_a_init
        ...
        || !router2.ready()) {
        return fail("net lab udp diag forward smoke router2 init failed\n", 3);
    }
    ...
    if (!server_init || !server_node.ready()) {
        return fail("net lab udp diag forward smoke server init failed\n", 4);
    }
    ...
    if (!bound_client
        ...
        return fail("net lab udp diag forward smoke bind failed\n", 5);
    }
    ...
    if (!ping
        ...
        return fail("net lab udp diag forward smoke ping send failed\n", 6);
    }
    ...
    if (!drive_until_idle(
            ...
            middle_link,
            server_link)) {
        return fail("net lab udp diag forward smoke ping exchange stalled\n", 7);
    }
    ...
    if (!client_target
        ...
        return fail("net lab udp diag forward smoke ping exchange failed\n", 8);
    }
    ...
    if (!count
        ...
        || client.queued_count() != 1u) {
        return fail("net lab udp diag forward smoke count send failed\n", 9);
    }
    ...
    if (!drive_until_idle(
            ...
            middle_link,
            server_link)) {
        return fail("net lab udp diag forward smoke count exchange stalled\n", 10);
    }

    if (!client_state.got_count
        || !server_state.saw_count
        || client.pending_count() != 0u
        || client.response_count() != 2u
        || server.request_count() != 2u
        || server.reply_count() != 2u
        || server.error_reply_count() != 0u
        || router1.forwarded_count() != 4u
        || router2.forwarded_count() != 4u
        || client_state.failed
        || client_state.error_calls != 0u
        || server_state.error_calls != 0u) {
        return fail("net lab udp diag forward smoke count exchange failed\n", 11);
    }
    ...
    if (!slow
        ...
        || client.queued_count() != 1u) {
        return fail("net lab udp diag forward smoke slow send failed\n", 12);
    }
    ...
    if (!drive_until_idle(
            ...
            middle_link,
            server_link)) {
        return fail("net lab udp diag forward smoke slow exchange stalled\n", 13);
    }

    if (!client_state.got_slow_count
        || !server_state.saw_slow_count
        || client.pending_count() != 0u
        || client.response_count() != 3u
        || server.request_count() != 3u
        || server.reply_count() != 3u
        || server.error_reply_count() != 0u
        || router1.forwarded_count() != 6u
        || router2.forwarded_count() != 6u
        || client_state.failed
        || client_state.error_calls != 0u
        || server_state.error_calls != 0u) {
        return fail("net lab udp diag forward smoke slow exchange failed\n", 14);
    }
    ...
    if (!meta
        || client.pending_count() != 1u
        || client.request_count() != 4u
        || client.queued_count() != 1u) {
        return fail("net lab udp diag forward smoke meta send failed\n", 15);
    }
    ...
    if (!drive_until_idle(
            ...
            middle_link,
            server_link)) {
        return fail("net lab udp diag forward smoke meta exchange stalled\n", 16);
    }

    if (!client_state.got_meta_unsupported
        || client.pending_count() != 0u
        || client.response_count() != 4u
        || client.last_error() != net::errc::ok
        || server.request_count() != 4u
        || server.reply_count() != 4u
        || server.error_reply_count() != 1u
        || server.queued_reply_count() != 1u
        || server.last_error() != net::errc::ok
        || router1.forwarded_count() != 8u
        || router2.forwarded_count() != 8u
        || router1.destination_unreachable_count() != 0u
        || router2.destination_unreachable_count() != 0u
        || router1.ttl_expired_count() != 0u
        || router2.ttl_expired_count() != 0u
        ...
        || client_state.error_calls != 0u
        || server_state.error_calls != 0u) {
        return fail("net lab udp diag forward smoke meta exchange failed\n", 17);
    }

    std::puts("net lab udp diag forward smoke: ok");
    return 0;
}
