...
int charm_posix_c_fs_header_entry(void) {
    ...
    *err = 114;
    if (charm_posix_stat(".", &st) != 0) return 649;
    if (*err != 114) return 650;
    if ((st.st_mode & CHARM_POSIX_S_IFMT) != CHARM_POSIX_S_IFDIR) return 651;
    *err = 115;
    ...
    *err = 116;
    fd = charm_posix_open("../parent.txt", CHARM_POSIX_O_RDONLY, 0);
    if (fd < 0) return 655;
    if (*err != 116) return 656;
    *err = 117;
    if (charm_posix_read(fd, buf, 2) != 2) return 657;
    if (*err != 117) return 658;
    if (memcmp(buf, "up", 2) != 0) return 659;
    *err = 118;
    ...
    *err = 119;
    if (charm_posix_rename("parent.txt", "parent-renamed.txt") != 0) return 662;
    if (*err != 119) return 663;
    *err = 0;
    if (charm_posix_stat("parent.txt", &st) != -1) return 664;
    if (*err != CHARM_POSIX_ENOENT) return 665;
    *err = 120;
    if (charm_posix_stat("parent-renamed.txt", &st) != 0) return 666;
    if (*err != 120) return 667;
    ...
    *err = 103;
    if (charm_posix_rename("sub", "sub-renamed") != 0) return 685;
    if (*err != 103) return 686;
    *err = 0;
    if (charm_posix_stat("sub", &st) != -1) return 672;
    if (*err != CHARM_POSIX_ENOENT) return 673;
    *err = 124;
    if (charm_posix_stat("sub-renamed", &st) != 0) return 687;
    if (*err != 124) return 688;
    if ((st.st_mode & CHARM_POSIX_S_IFMT) != CHARM_POSIX_S_IFDIR) return 689;
    *err = 126;
    if (charm_posix_chdir("sub-renamed") != 0) return 692;
    if (*err != 126) return 693;
    *err = 127;
    if (charm_posix_getcwd(cwd, sizeof(cwd)) != cwd) return 694;
    if (*err != 127) return 695;
    if (strcmp(cwd, "/cfs/sub-renamed") != 0) return 696;
    *err = 128;
    dir = charm_posix_opendir(".");
    if (dir == 0) return 697;
    if (*err != 128) return 698;
    *err = 129;
    if (charm_posix_readdir(dir) != 0) return 699;
    if (*err != 129) return 700;
    *err = 130;
    if (charm_posix_closedir(dir) != 0) return 701;
    if (*err != 130) return 702;
    *err = 131;
    if (charm_posix_chdir("..") != 0) return 703;
    if (*err != 131) return 704;
    *err = 132;
    if (charm_posix_getcwd(cwd, sizeof(cwd)) != cwd) return 705;
    if (*err != 132) return 706;
    if (strcmp(cwd, "/cfs") != 0) return 707;
    *err = 125;
    if (charm_posix_rmdir("sub-renamed") != 0) return 618;
    if (*err != 125) return 619;
    *err = 0;
    if (charm_posix_stat("sub-renamed", &st) != -1) return 690;
    if (*err != CHARM_POSIX_ENOENT) return 691;
