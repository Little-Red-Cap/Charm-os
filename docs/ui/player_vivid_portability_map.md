...
- Board-port layer: `player.runtime.hqzy_cm7.player_ui_port_bridge` is the H747-local seed for framebuffer, dirty region, touch, and clock facts before a future board shell calls `make_player_board_runtime()`. Its local probe validates the facts and callback order without importing the MD3/Vivid runtime stack.
...
- `player.font_resource` is now the app-common font input contract, and `player.font_resource_apply` is the matching app-common application seam. Windows preview and board assembly both build `FontResourceConfig` through the same builtin/file/package helpers, while `player.app` delegates application to the controller instead of open-coding file-font branches.
...
- `audio.spectrum` keeps FFT/host math details behind a small analyzer backend so `audio.player` can keep playback independent from spectrum visualization.
...
- The next cleanup order should be a non-Windows board shell target that calls `make_player_board_runtime()`; the H747-local port bridge is the seed for that path. After that, CMSIS-DSP or fixed-point spectrum backend, concrete `FontResourceKind::Package` binary/layout, time/diagnostics provider, then controller dynamic state slimming.
...
- `--ui-ci` should prove that a cover resource binding or board-style static record table can be installed through runtime/board assembly and consumed by the real Now Playing cover path without reintroducing host decode assumptions.
