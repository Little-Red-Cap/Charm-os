...
- `net.icmp` 已补上最小 echo codec/service/egress，并接入 `IcmpStackPump`：`Stack -> Ipv4Service -> IcmpEchoService` ingress 可解析 echo request/reply，ARP 解析后可完成 ICMP echo request 发送，`net_icmp_smoke` 可回归 codec、unsupported drop、checksum 校验与 ARP 后发送成功，`net_icmp_roundtrip_smoke` 可回归 `ARP -> echo request -> echo reply` 的最小真实往返闭环
