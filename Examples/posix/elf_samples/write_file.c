...
int entry(int argc, char** argv, char** envp) {
    ...
    if (st.st_size != 0) {
        write_str(2, "truncate fail\n");
        close(fd);
        return 15;
    }

    if (write(fd, payload, payload_len) != (int)payload_len) {
        write_str(2, "write fail\n");
        close(fd);
        return 16;
    }
    ...
    if (!bytes_equal(buf, payload, payload_len)) {
