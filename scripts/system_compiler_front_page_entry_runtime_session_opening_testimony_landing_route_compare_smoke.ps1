param(
    ...
    [string]$OutputRoot = "cmake-build-system-compiler-front-page-entry-runtime-session-opening-testimony-landing-route-compare-smoke",
    [string]$PythonExe = "",
    [switch]$Clean
)
...
try {
    ...
    if (
        ...
        Write-Host "[RUNTIME-SESSION-OPENING-TESTIMONY-LANDING-ROUTE-COMPARE-SMOKE] route_bootstrap=reuse-existing"
    } else {
        Invoke-ExternalTool `
    ...
    foreach ($case in $cases) {
        ...
        Invoke-ExternalTool `
            ...
            -FailureMessage ("runtime session front page route compare export failed for case '{0}'" -f $case.Name)
        ...
        Invoke-ExternalTool `
            ...
            -FailureMessage ("runtime session front page route compare validation failed for case '{0}'" -f $case.Name)
        ...
        Write-Host (
            "[RUNTIME-SESSION-OPENING-TESTIMONY-LANDING-ROUTE-COMPARE-SMOKE] case={0} verdict={1} changed={2} level1=+[{3}] -[{4}]" -f
            $case.Name,
...
Write-Host ("[RUNTIME-SESSION-OPENING-TESTIMONY-LANDING-ROUTE-COMPARE-SMOKE] output_root={0}" -f $outputRootPath)
