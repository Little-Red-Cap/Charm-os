...
export module player.stm32h7.fs_demo_mmc;
...
#define CHARM_SDMMC_HANDLE hmmc1
#define CHARM_SDMMC_CARD_INFO HAL_MMC_CardInfoTypeDef
#define CHARM_SDMMC_CARD_STATE_TRANSFER HAL_MMC_CARD_TRANSFER
#define CHARM_SDMMC_STATE_RESET HAL_MMC_STATE_RESET
#define CHARM_SDMMC_GET_ERROR(handle) HAL_MMC_GetError(handle)
#define CHARM_SDMMC_GET_STATE(handle) HAL_MMC_GetCardState(handle)
#define CHARM_SDMMC_INIT_HANDLE(handle) HAL_MMC_Init(handle)
#define CHARM_SDMMC_GET_INFO(handle, info) HAL_MMC_GetCardInfo(handle, info)
#define CHARM_SDMMC_CONFIG_WIDE(handle, mode) HAL_MMC_ConfigWideBusOperation(handle, mode)
#define CHARM_SDMMC_READ(handle, buf, lba, count, timeout) HAL_MMC_ReadBlocks(handle, buf, lba, count, timeout)
#define CHARM_SDMMC_READ_DMA(handle, buf, lba, count) HAL_MMC_ReadBlocks_DMA(handle, buf, lba, count)
#define CHARM_SDMMC_WRITE(handle, buf, lba, count, timeout) HAL_MMC_WriteBlocks(handle, buf, lba, count, timeout)
#define CHARM_SDMMC_WRITE_DMA(handle, buf, lba, count) HAL_MMC_WriteBlocks_DMA(handle, buf, lba, count)

static MMC_HandleTypeDef& card = CHARM_SDMMC_HANDLE;
...
namespace {
    ...
    void sdmmc_log_init_fail(const char* stage) noexcept {
        const auto err = static_cast<util::u32>(HAL_MMC_GetError(&hmmc1));
        const auto clk = static_cast<util::u32>(HAL_RCCEx_GetPeriphCLKFreq(RCC_PERIPHCLK_SDMMC));
        const auto clkcr = static_cast<util::u32>(hmmc1.Instance->CLKCR);
        const auto sta = static_cast<util::u32>(hmmc1.Instance->STA);
        const auto power = static_cast<util::u32>(hmmc1.Instance->POWER);
        const auto cmd = static_cast<util::u32>(hmmc1.Instance->CMD);
        const auto arg = static_cast<util::u32>(hmmc1.Instance->ARG);
        const auto resp1 = static_cast<util::u32>(hmmc1.Instance->RESP1);
        log<"fs sdmmc: {} fail err=0x{:08X} ker_ck={}Hz clkcr=0x{:08X}">(
            stage, err, clk, clkcr);
        log<"fs sdmmc: {} sta=0x{:08X} power=0x{:08X} cmd=0x{:08X} arg=0x{:08X} resp1=0x{:08X}">(
            stage, sta, power, cmd, arg, resp1);
    }

    bool sdmmc_hw_init() noexcept {
        ...
        const auto st = HAL_MMC_Init(&hmmc1);
        if (st != HAL_OK) {
            const auto err = static_cast<util::u32>(HAL_MMC_GetError(&hmmc1));
            log<"fs sdmmc: HAL_MMC_Init failed st={} err=0x{:08X}">(
                static_cast<int>(st), err);
            sdmmc_log_init_fail("mmc_init");
            return false;
        }
        return true;
    }
...
#if 0
    void sdmmc_diag_after_fail() noexcept {
        log<"fs sdmmc: diag not supported for emmc">();
        return;
    ...
    struct SdBlockDevice {
        bool init() noexcept {
            ...
            log<"fs sdmmc: ker_ck={}Hz clkcr=0x{:08X}">(
                static_cast<util::u32>(HAL_RCCEx_GetPeriphCLKFreq(RCC_PERIPHCLK_SDMMC)),
                static_cast<util::u32>(card.Instance->CLKCR));
