...
public:
    ...
    static constexpr int kPopupPadding = 2;
    ...
    void open(int x, int y, int w) {
        ...
        popup->set_rect({x, y, w, visible * kItemHeight + kPopupPadding * 4});
        clear_list_resources();
        rebuild_list();
...
private:
    int calc_index(int y) const noexcept {
        auto* popup = factory_.get_container(popup_h_);
        if (!popup) return -1;
        int scroll_y = 0;
        ...
        return (y - popup->get_rect().y - kPopupPadding + scroll_y) / kItemHeight;
    }
    ...
    void rebuild_list() {
        ...
        if (auto* sc = factory_.get_scroll_container(scroll_h_)) {
            sc->set_rect({kPopupPadding, kPopupPadding,
                          popup->get_rect().w - kPopupPadding * 2,
                          visible * kItemHeight + kPopupPadding * 2});
            factory_.link(popup_h_, scroll_h_);
        }

        list_h_ = factory_.create_list();
        if (auto* list = factory_.get_list(list_h_)) {
            list->set_rect({0, 0, popup->get_rect().w - kPopupPadding * 2, option_count_ * kItemHeight});
            ...
            for (int i = 0; i < option_count_; ++i) {
                ...
                if (auto* item = factory_.get_list_item(item_hndl)) {
                    item->set_size(list->get_rect().w, kItemHeight);
    ...
    void clear_list_resources() {
        if (scroll_h_) {
            factory_.destroy_tree(scroll_h_);
        }
        scroll_h_ = {};
        list_h_ = {};
    }
