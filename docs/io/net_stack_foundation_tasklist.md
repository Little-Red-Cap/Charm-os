...
- `net.ipv4` 已补上原始 IPv4 datagram 的通用 egress / TTL 重写 / 挂起队列，`net.forward` 已提供最小双口 IPv4 forwarding hop，并通过 proxy ARP 把“多跳但仍保持现有 host pump 调用面”的实验路径接通：`net_lab_forward_trace_smoke` 可覆盖 `client -> router1 -> router2 -> destination` 的真实转发、TTL 递减、`ICMP time exceeded` 与最终 `echo reply` 闭环
