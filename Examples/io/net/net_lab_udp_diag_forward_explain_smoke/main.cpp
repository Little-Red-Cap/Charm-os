...
namespace {
    ...
    [[nodiscard]] bool same_request(const net::diag::ForwardExplainRequest& lhs,
                                    const net::diag::ForwardExplainRequest& rhs) noexcept {
    ...
    [[nodiscard]] bool same_reply(const net::diag::ForwardExplainReply& reply,
    ...
    struct ClientState {
        ...
        net::diag::ForwardExplainReply reply{};
        ...
        static void on_forward_explain(void* ctx,
                                       util::u16 request_id,
                                       net::diag::udp::Status status,
                                       const net::diag::ForwardExplainReply& response) noexcept {
    ...
    struct ServerState {
        ...
        bool saw_explain{false};
        util::usize explain_calls{0};
        util::usize error_calls{0};
        net::errc last_error{net::errc::ok};
        net::diag::ForwardExplainRequest last_request{};
        ...
        static net::diag::udp::Status on_forward_explain(
            void* ctx,
            const net::diag::ForwardExplainRequest& request,
            net::diag::ForwardExplainReply& response) noexcept {
            auto* self = static_cast<ServerState*>(ctx);
            if (!self || self->router == nullptr) {
                return net::diag::udp::Status::internal_error;
            }

            self->saw_explain = true;
            ++self->explain_calls;
            ...
            response = net::diag::ForwardExplainReply::from_snapshot(snapshot.value());
    ...
    [[nodiscard]] bool service_step(Client& client,
        ...
        if (!client_progress) {
            std::fprintf(stderr,
                         "udp diag forward explain client service error=%d\n",
        ...
        if (!router1_progress) {
            std::fprintf(stderr,
                         "udp diag forward explain router1 service error=%d\n",
        ...
        if (!router2_progress) {
            std::fprintf(stderr,
                         "udp diag forward explain router2 service error=%d\n",
        ...
        if (!server_progress) {
            std::fprintf(stderr,
                         "udp diag forward explain server service error=%d\n",
...
int main() {
    constexpr auto client_mac = net::MacAddress::from_bytes(0x02u, 0x3Du, 0x00u, 0x00u, 0x00u, 0x02u);
    constexpr auto router1_a_mac = net::MacAddress::from_bytes(0x02u, 0x3Du, 0x00u, 0x00u, 0x00u, 0x11u);
    constexpr auto router1_b_mac = net::MacAddress::from_bytes(0x02u, 0x3Du, 0x00u, 0x00u, 0x00u, 0x12u);
    constexpr auto router2_a_mac = net::MacAddress::from_bytes(0x02u, 0x3Du, 0x00u, 0x00u, 0x00u, 0x21u);
    constexpr auto router2_b_mac = net::MacAddress::from_bytes(0x02u, 0x3Du, 0x00u, 0x00u, 0x00u, 0x22u);
    constexpr auto server_mac = net::MacAddress::from_bytes(0x02u, 0x3Du, 0x00u, 0x00u, 0x00u, 0x09u);
    ...
    if (!client_init || !client_node.ready()) {
        return fail("net lab udp diag forward explain smoke client init failed\n", 1);
    }
    ...
    if (!router1_a_init
        ...
        || !router1.ready()) {
        return fail("net lab udp diag forward explain smoke router1 init failed\n", 2);
    }
    ...
    if (!router2_a_init
        ...
        || !router2.ready()) {
        return fail("net lab udp diag forward explain smoke router2 init failed\n", 3);
    }
    ...
    if (!server_init || !server_node.ready()) {
        return fail("net lab udp diag forward explain smoke server init failed\n", 4);
    }
    ...
    auto registered_forward_explain = server.on_forward_explain(
        &ServerState::on_forward_explain,
        &server_state);
    if (!bound_client
        || !bound_server
        || !registered_forward_explain
        || !server.has_forward_explain()
        ...
        return fail("net lab udp diag forward explain smoke bind failed\n", 5);
    }
    ...
    auto run_query = [&](const char* send_fail,
        ...
        const auto explain_calls_before = server_state.explain_calls;
        const auto before = capture_phase(router1, router2, server);
        auto sent = client.query_forward_explain(
            request,
            client_link.now_ticks(),
            timeout_ms,
            &ClientState::on_forward_explain,
        ...
        if (!client_state.got_reply
            ...
            || !server_state.saw_explain
            || server_state.explain_calls != (explain_calls_before + 1u)
    ...
    const auto explicit_request = net::diag::ForwardExplainRequest::ipv4(
        net::Ipv4ForwardingPort::a,
        server_ip,
        64u);
    if (!run_query(
            "net lab udp diag forward explain smoke explicit send failed\n",
            "net lab udp diag forward explain smoke explicit stalled\n",
            "net lab udp diag forward explain smoke explicit mismatch\n",
            explicit_request,
            20u,
            1u,
            1u,
            [&](const net::diag::ForwardExplainReply& reply) noexcept {
    ...
    if (!client_target
        ...
        return fail("net lab udp diag forward explain smoke arp prime mismatch\n", 7);
    }

    const auto connected_request = net::diag::ForwardExplainRequest::ipv4(
        net::Ipv4ForwardingPort::a,
        router2_a_ip,
        64u);
    if (!run_query(
            "net lab udp diag forward explain smoke connected send failed\n",
            "net lab udp diag forward explain smoke connected stalled\n",
            "net lab udp diag forward explain smoke connected mismatch\n",
            connected_request,
            20u,
            0u,
            0u,
    ...
    const auto local_request = net::diag::ForwardExplainRequest::ipv4(
        net::Ipv4ForwardingPort::b,
        router1_a_ip,
        64u);
    if (!run_query(
            "net lab udp diag forward explain smoke local send failed\n",
            "net lab udp diag forward explain smoke local stalled\n",
            "net lab udp diag forward explain smoke local mismatch\n",
            local_request,
            20u,
            0u,
            0u,
            [&](const net::diag::ForwardExplainReply& reply) noexcept {
                return reply.local_dropped()
    ...
    const auto broadcast_request = net::diag::ForwardExplainRequest::ipv4(
        net::Ipv4ForwardingPort::a,
        net::IpAddress::ipv4_broadcast(),
        64u);
    if (!run_query(
            "net lab udp diag forward explain smoke broadcast send failed\n",
            "net lab udp diag forward explain smoke broadcast stalled\n",
            "net lab udp diag forward explain smoke broadcast mismatch\n",
            broadcast_request,
            20u,
            0u,
            0u,
    ...
    const auto ttl_request = net::diag::ForwardExplainRequest::ipv4(
        net::Ipv4ForwardingPort::a,
        server_ip,
        1u);
    if (!run_query(
            "net lab udp diag forward explain smoke ttl send failed\n",
            "net lab udp diag forward explain smoke ttl stalled\n",
            "net lab udp diag forward explain smoke ttl mismatch\n",
            ttl_request,
            20u,
            0u,
            0u,
            [&](const net::diag::ForwardExplainReply& reply) noexcept {
                return reply.ttl_expired()
                    && !reply.has_egress_port()
                    && !reply.has_decision_snapshot()
                    && reply.emits_icmp_error()
                    && same_reply(
    ...
    const auto no_route_request = net::diag::ForwardExplainRequest::ipv4(
        net::Ipv4ForwardingPort::a,
        no_route_ip,
        64u);
    if (!run_query(
            "net lab udp diag forward explain smoke no-route send failed\n",
            "net lab udp diag forward explain smoke no-route stalled\n",
            "net lab udp diag forward explain smoke no-route mismatch\n",
            no_route_request,
            20u,
            0u,
            0u,
    ...
    auto added_direct = router1.add_direct_route(
        direct_network,
        24u,
        net::Ipv4ForwardingPort::a,
        2u);
    if (!added_direct || router1.route_count() != 2u) {
        return fail("net lab udp diag forward explain smoke add direct failed\n", 13);
    }

    const auto direct_request = net::diag::ForwardExplainRequest::ipv4(
        net::Ipv4ForwardingPort::b,
        direct_ip,
        64u);
    if (!run_query(
            "net lab udp diag forward explain smoke direct send failed\n",
            "net lab udp diag forward explain smoke direct stalled\n",
            "net lab udp diag forward explain smoke direct mismatch\n",
            direct_request,
            20u,
            0u,
            0u,
    ...
    if (!router1.remove_route(direct_network, 24u) || router1.route_count() != 1u) {
        return fail("net lab udp diag forward explain smoke remove direct failed\n", 15);
    }

    if (!run_query(
            "net lab udp diag forward explain smoke fallback send failed\n",
            "net lab udp diag forward explain smoke fallback stalled\n",
            "net lab udp diag forward explain smoke fallback mismatch\n",
            direct_request,
            20u,
            0u,
            0u,
    ...
    std::puts("net lab udp diag forward explain smoke: ok");
    return 0;
}
