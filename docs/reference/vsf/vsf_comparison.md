...
如果某条结论已经被 Charm 自己的契约、总览或专题入口吸收，优先回到相应上位文档，不要把本页当成当前路线图。
...
- 在 [`docs/architecture/dependency_whitelist.md`](../../architecture/dependency_whitelist.md) 中补组件依赖关系
...
- 为每类 HAL 外设提供“驱动实现范围”与“平台 hook 范围”说明
- 形成最小模板与示例

## 5. MAL 抽象（Memory Abstraction Layer）（可借鉴）

- VSF 的 MAL 把 block / file / flash 统一为设备抽象
- 对当前的 VFS + FatFs + BlockDevice 很有收益
