param(
    ...
    [string]$OutputRoot = "cmake-build-system-compiler-front-page-entry-runtime-session-opening-testimony-landing-route-smoke",
    [string]$PythonExe = "",
    [switch]$Clean
)
...
$landingCompareSmokeScript = Join-Path $PSScriptRoot "system_compiler_front_page_entry_runtime_session_opening_testimony_landing_compare_smoke.ps1"
...
foreach ($requiredPath in @($landingSmokeScript, $landingCompareSmokeScript, $routeExportScript, $routeValidateScript)) {
...
try {
    ...
    if ((-not $Clean) -and (Test-Path -LiteralPath $cleanLandingPath) -and (Test-Path -LiteralPath $driftLandingPath)) {
        Write-Host "[RUNTIME-SESSION-OPENING-TESTIMONY-LANDING-ROUTE-SMOKE] landing_bootstrap=reuse-existing"
    } else {
        Invoke-ExternalTool `
    ...
    $compareLandingPath = Join-Path $runtimeSessionLandingCompareRootPath "opening-testimony-landing-clean-to-drift\front-page.entry-opening-testimony.landing.compare.summary.json"
    if ((-not $Clean) -and (Test-Path -LiteralPath $compareLandingPath)) {
        Write-Host "[RUNTIME-SESSION-OPENING-TESTIMONY-LANDING-ROUTE-SMOKE] compare_bootstrap=reuse-existing"
    } else {
        Invoke-ExternalTool `
            -Executable $powerShellExe `
            -ArgumentList @(
                ...
                "-RuntimeSessionLandingRoot",
                $runtimeSessionLandingRootPath,
                "-OutputRoot",
                $runtimeSessionLandingCompareRootPath,
                "-PythonExe",
                $resolvedPythonExe,
                "-Clean"
            ) `
            -FailureMessage "runtime session opening testimony landing compare smoke bootstrap failed"
    }
    ...
    foreach ($case in $cases) {
        ...
        Invoke-ExternalTool `
            ...
            -FailureMessage ("runtime session front page route export failed for case '{0}'" -f $case.Name)
        ...
        Invoke-ExternalTool `
            ...
            -FailureMessage ("runtime session front page route validation failed for case '{0}'" -f $case.Name)
        ...
        Write-Host (
            "[RUNTIME-SESSION-OPENING-TESTIMONY-LANDING-ROUTE-SMOKE] case={0} entries={1} level1={2} root_schema={3}" -f
            $case.Name,
...
Write-Host ("[RUNTIME-SESSION-OPENING-TESTIMONY-LANDING-ROUTE-SMOKE] output_root={0}" -f $outputRootPath)
