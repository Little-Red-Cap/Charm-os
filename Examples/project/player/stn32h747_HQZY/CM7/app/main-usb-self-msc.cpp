...
struct UsbBlockTrace {
    ...
    fs::Status read(util::u64 lba, std::span<util::u8> data) noexcept {
        ...
        if (last_read_ok && data.size() >= last_read_head.size()) {
            std::memcpy(last_read_head.data(), data.data(), last_read_head.size());
        }
...
util::u16 le16(const std::array<usb::u8, 512>& sector, std::size_t off) noexcept {
    return static_cast<util::u16>(sector[off] | (sector[off + 1] << 8));
}

util::u32 le32(const std::array<usb::u8, 512>& sector, std::size_t off) noexcept {
    return static_cast<util::u32>(sector[off]
        | (sector[off + 1] << 8)
        | (sector[off + 2] << 16)
        | (sector[off + 3] << 24));
}
...
int charm_player_selected_profile_main() {
    ...
    if ((kDumpLba0 || kUsbMscUseBpbCapacity) && dev->read && dev->block_size <= 512 && dev->block_size != 0) {
        ...
        if (st) {
            ...
            if (kDumpLba0) {
                ...
                out::println<"bpb: bps={} spc={} rsvd={} fats={} root_ent={} tot16={} media=0x{:02X} fatsz16={} spt={} heads={} hidden={} tot32={} fatsz32={} rootcl={} fsinfo={} bkboot={} bpb_total={}">(
                    static_cast<unsigned>(le16(lba0, 11)),
                    static_cast<unsigned>(lba0[13]),
                    static_cast<unsigned>(le16(lba0, 14)),
                    static_cast<unsigned>(lba0[16]),
                    static_cast<unsigned>(le16(lba0, 17)),
                    static_cast<unsigned>(le16(lba0, 19)),
                    static_cast<unsigned>(lba0[21]),
                    static_cast<unsigned>(le16(lba0, 22)),
                    static_cast<unsigned>(le16(lba0, 24)),
                    static_cast<unsigned>(le16(lba0, 26)),
                    static_cast<unsigned long>(le32(lba0, 28)),
                    static_cast<unsigned long>(le32(lba0, 32)),
                    static_cast<unsigned long>(le32(lba0, 36)),
                    static_cast<unsigned long>(le32(lba0, 44)),
                    static_cast<unsigned>(le16(lba0, 48)),
                    static_cast<unsigned>(le16(lba0, 50)),
                    static_cast<unsigned long>(bpb_sectors));
            }
        } else if (kDumpLba0) {
    ...
    out::println<"usb: storage block_size={} blocks={} removable={} ro={}">(
    ...
    while (true) {
        ...
        if ((now - last_ms) >= 1000u) {
            ...
            if (binding.bot) {
                ...
                out::println<"msc: cbw={} cmd=0x{:02X} lun={} xfer={} flags=0x{:02X} tag=0x{:08X} scsi=0x{:02X} st={} lba={} blocks={} bsize={} sense={:02X}/{:02X}/{:02X} rc={} rf={} rc_lba={} rc_blocks={} rc_bsize={} rf_blocks={} rf_bsize={} csw={} csw_st={} csw_res={} csw_tag=0x{:08X} phase={} in={} wait={} clr={} clr_in={} rearm={}">(
                    bot.cbw_count(),
                    bot.last_cbw_cmd(),
                    bot.last_cbw_lun(),
                    ...
                    bot.phase_code(),
                    bot.last_in_result(),
                    bot.stall_wait_csw() ? 1u : 0u,
                    static_cast<unsigned long>(bot.clear_stall_count()),
                    bot.last_clear_stall_in_ep() ? 1u : 0u,
                    bot.out_rearm_pending() ? 1u : 0u);
            }
            if (kDumpMscIo) {
                ...
                out::println<"msc: io head={} {} {} {} {} {} {} {} {} {} {} {} {} {} {} {}">(
                    usb_trace.last_read_head[0],
                    usb_trace.last_read_head[1],
                    usb_trace.last_read_head[2],
                    usb_trace.last_read_head[3],
                    usb_trace.last_read_head[4],
                    usb_trace.last_read_head[5],
                    usb_trace.last_read_head[6],
                    usb_trace.last_read_head[7],
                    usb_trace.last_read_head[8],
                    usb_trace.last_read_head[9],
                    usb_trace.last_read_head[10],
                    usb_trace.last_read_head[11],
                    usb_trace.last_read_head[12],
                    usb_trace.last_read_head[13],
                    usb_trace.last_read_head[14],
                    usb_trace.last_read_head[15]);
            }
