...
public:
  ...
  void present(const gui::Canvas1bpp<W, H>& c) {
    // 1bpp buffer -> ARGB8888
    // 白色=on，黑色=off（你也可以反过来）
    ...
    const int stride = c.strideBytes();      // 每行字节数

    auto* dst = pixels_.data();              // uint32_t*, 大小 W*H
    ...
    /*
    * 什么时候选 Lock，什么时候选 Update？
    * 1.已经有一份连续的 CPU 像素缓冲（pixels_）——UpdateTexture 最简单
    * 2.想直接写进 texture 的内存、避免一次额外拷贝——才考虑 Lock（但各平台支持差异更大）
    * 以现在“PC 做验证台”的定位，UpdateTexture 更合适。
     */
#if 0
    ...
    // pitch 可能 != W*4，逐行拷贝
    auto* dst = static_cast<std::uint8_t*>(texPixels);
    const auto* src = reinterpret_cast<const std::uint8_t*>(pixels_.data());
    const int srcPitch = W * 4;

    for (int y = 0; y < H; ++y) {
      std::memcpy(dst + y * pitch, src + y * srcPitch, (size_t)srcPitch);
    }
