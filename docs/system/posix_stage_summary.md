...
- newlib cwd smoke now also pins relative `access()` directly: `access("child.txt", F_OK)` and `access("sub", X_OK)` resolve against the live cwd without clobbering `errno`, while a missing relative path still reports `ENOENT`
- Charm now ships a local `dirent.h` shim on the POSIX include path because the current `arm-none-eabi` newlib baseline reports `<dirent.h> not supported`; the newlib bridge now exports `opendir/readdir/closedir` on top of `posix::user`, including EOF-vs-error `errno` behavior and minimum `d_name/d_type/d_mode/d_size` payloads
- newlib cwd smoke now also pins relative rename/cleanup directly: `rename("parent.txt", "parent-renamed.txt")`, `unlink("parent-renamed.txt")`, `rename("sub", "sub-renamed")`, and `rmdir("sub-renamed")` all resolve against the live cwd and leave the old relative paths reporting `ENOENT`
- newlib cwd smoke now also pins relative `remove()` success directly: `remove("child.txt")` and `remove("remove-dir")` both resolve against the live cwd, remove the target path, and keep success-side `errno` unchanged
- newlib cwd smoke now also pins renamed-directory navigation directly: after `rename("sub", "sub-renamed")`, relative `chdir("sub-renamed")`, `getcwd`, and `chdir("..")` continue to behave against the new directory name without clobbering `errno`
- newlib cwd smoke now also pins standard directory traversal directly through the shimmed `dirent` surface: `opendir("sub")`, `opendir(".")`, `opendir("..")`, `readdir()`, and `closedir()` resolve against the live cwd, preserve `errno` on EOF/success, expose minimum file-vs-dir shape through `d_type`, and keep renamed empty-directory traversal stable after `chdir("sub-renamed")`
