...
- `Examples/ui/vivid/semantic_action_request_demo`: verifies Semantic Action Request v0, including controlled semantic activate execution, focus preparation event trace, emitted click evidence, unsupported/focus-scope rejection, duplicate id, missing id, and request ledger evidence; stdout follows [`vivid_evidence_stdout_law.md`](vivid_evidence_stdout_law.md), guarded by CTest final `result=ok cases=8`.
