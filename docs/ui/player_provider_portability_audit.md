...
| Font | `player.font_resource` owns the app-common font input helpers, `FontResourceConfig` carries explicit font source kind plus sizes, `player.font_resource_apply` owns the app-common application seam, host preview composes file-font config through the same helpers, and board ports can pass package bytes through `PlayerFontPackageResourceView`. | `CHARM_PLAYER_HOST_FILE_FONTS`, `CHARM_PLAYER_PC_FONT_CACHE`; profile log prints `host_file_fonts`. | FreeType/VFS font buffers, Win32/GDI glyph cache vectors, preview argv strings. | Define the concrete board font package binary/layout behind `FontResourceKind::Package`. |
...
- `PlayerFontPackageResourceView` gives board code a named place to pass package bytes/key/size metadata without pretending file paths exist, `player.font_resource` provides the common config builders for builtin/file/package modes, and `player.font_resource_apply` is now the single app-common seam that hands those semantic inputs to the controller/UI side.
...
- `player.app` no longer checks `has_file_resource()` or calls `set_font_config(...)` directly, so file-font details are no longer part of the app lifecycle shell.
