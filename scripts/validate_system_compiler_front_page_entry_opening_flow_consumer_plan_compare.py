...
from compare_system_compiler_front_page_entry_opening_flow_consumer_plan import build_compare_summary_model
from system_compiler_front_page_route_lib import load_json, normalize_path


COMPARE_SCHEMA_PATH = "schemas/system_compiler.front_page_entry_opening_flow_consumer_plan_compare.v0.schema.json"
...
def validate_references(summary: dict, errors: list[str]) -> None:
    ...
    for field in (
        "baseline_plan_summary_path",
        "candidate_plan_summary_path",
        "output_root",
    ...
    for index, provenance in enumerate(summary.get("plan_provenance", [])):
        if not isinstance(provenance, dict):
            errors.append(f"plan_provenance[{index}]: invalid provenance entry")
            continue
        ensure_exists(provenance.get("source_summary_path"), f"plan_provenance[{index}].source_summary_path", errors)
        ensure_exists(
            provenance.get("source_report_markdown_path"),
            f"plan_provenance[{index}].source_report_markdown_path",
            errors,
        )
        ensure_exists(
            provenance.get("source_check_text_path"),
            f"plan_provenance[{index}].source_check_text_path",
            errors,
        )

    for index, change in enumerate(summary.get("plan_action_changes", [])):
        if not isinstance(change, dict):
            errors.append(f"plan_action_changes[{index}]: invalid action change")
            continue
        ensure_exists(
            change.get("baseline_target_summary_path"),
            f"plan_action_changes[{index}].baseline_target_summary_path",
            errors,
            required=False,
        )
        ensure_exists(
            change.get("candidate_target_summary_path"),
            f"plan_action_changes[{index}].candidate_target_summary_path",
            errors,
            required=False,
        )
        ensure_exists(
            change.get("baseline_opener_summary_path"),
            f"plan_action_changes[{index}].baseline_opener_summary_path",
            errors,
            required=False,
        )
        ensure_exists(
            change.get("candidate_opener_summary_path"),
            f"plan_action_changes[{index}].candidate_opener_summary_path",
            errors,
            required=False,
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate system compiler front_page entry opening-flow consumer plan compare summary."
    )
    parser.add_argument(
        "--summary",
        default="",
        help=(
            "Path to plan compare summary JSON. If omitted, "
            "--bundle-root/front-page.entry-opening-flow.consumer.plan.compare.summary.json is used."
        ),
    )
    parser.add_argument(
        "--bundle-root",
        default="",
        help="Bundle root containing front-page.entry-opening-flow.consumer.plan.compare.summary.json.",
    )
    ...
    if args.summary:
        ...
        bundle_root = Path(args.bundle_root or "out/system-compiler-front-page-entry-opening-flow-consumer-plan-compare").resolve()
        summary_path = bundle_root / "front-page.entry-opening-flow.consumer.plan.compare.summary.json"

    try:
        import jsonschema

        summary = load_json(summary_path)
        compare_schema = load_json((repo_root / COMPARE_SCHEMA_PATH).resolve())
        plan_schema = load_json((repo_root / PLAN_SCHEMA_PATH).resolve())
        ...
        artifact_context = summary.get("artifact_context", {})
        baseline_plan_path = Path(artifact_context.get("baseline_plan_summary_path", "")).resolve()
        candidate_plan_path = Path(artifact_context.get("candidate_plan_summary_path", "")).resolve()
        ...
        jsonschema.validate(load_json(baseline_plan_path), plan_schema)
        jsonschema.validate(load_json(candidate_plan_path), plan_schema)
    ...
    print(f"[OK] plan verdict -> {summary.get('plan_verdict', '')}")
    print(f"[OK] changed actions -> {summary.get('plan_action_summary', {}).get('changed_action_count', 0)}")
    return 0
