...
export module alg_virtual_list;

export namespace alg::virtual_list {
    struct Window {
        int start{0};
        int visible{0};
        int offset_y{0};
    ...
    inline Window compute_window(int scroll_y,
                                 int row_height,
                                 int view_h,
                                 int base_y,
    ...
    public:
        ...
        const Slot& slot(std::size_t index) const noexcept { return slots_[index]; }

    private:
        Slot slots_[kMax]{};
    };
} // namespace alg::virtual_list
