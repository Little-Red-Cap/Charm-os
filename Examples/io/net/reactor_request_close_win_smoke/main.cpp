...
int main() {
    ...
    if (!winsock.ok) {
        std::fputs("reactor request close win winsock startup failed\n", stderr);
        return 1;
    }

    NativeSocket listener{::socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)};
    if (!listener.valid()) {
        std::fputs("reactor request close win native listen socket failed\n", stderr);
        return 2;
    }

    if (!set_nonblocking(listener.value)) {
        std::fputs("reactor request close win native listen nonblocking failed\n", stderr);
        return 3;
    }
    ...
    util::u16 port = 0;
    for (util::u16 candidate = 33520; candidate < 33620; ++candidate) {
        sockaddr_in addr{};
        ...
        if (::listen(listener.value, 2) != 0) {
            std::fputs("reactor request close win native listen failed\n", stderr);
            return 4;
        }
        port = candidate;
        break;
    ...
    if (port == 0) {
        std::fputs("reactor request close win native bind failed\n", stderr);
        return 5;
    }
    ...
    if (!client.connect(stack, net::Endpoint::ipv4_loopback(port))) {
        std::fputs("reactor request close win charm client connect failed\n", stderr);
        return 6;
    }
    ...
    if (!accepted || accept_error != 0 || !server.valid()) {
        std::fputs("reactor request close win native accept failed\n", stderr);
        return 7;
    }
    ...
    if (!client_driver.start()) {
        std::fputs("reactor request close win driver start failed\n", stderr);
        return 8;
    }
    ...
    auto request = client_session.send_request(
        ...
        &client_state);
    if (!request) {
        std::fputs("reactor request close win send_request failed\n", stderr);
        return 9;
    }
    ...
    if (!got_wire || native_error != 0 || wire_filled != static_cast<int>(wire.size())) {
        std::fputs("reactor request close win native recv failed\n", stderr);
        return 10;
    }
    if (wire[0] != 0x00u
        || wire[1] != 0x08u
        ...
        || wire[8] != 'l'
        || wire[9] != 'd') {
        std::fputs("reactor request close win wire mismatch\n", stderr);
        return 11;
    }
    ...
    if (!done) {
        std::fputs("reactor request close win timeout\n", stderr);
        return 12;
    }
    if (client_state.response_count != 0) {
        std::fputs("reactor request close win unexpected response\n", stderr);
        return 13;
    }
    if (client_state.timeout_count != 0) {
        std::fputs("reactor request close win unexpected timeout\n", stderr);
        return 14;
    }
    if (late_request || late_request.error() != net::errc::closed) {
        std::fputs("reactor request close win late send_request not rejected\n", stderr);
        return 15;
    }
    if (client_session.pending_count() != 0) {
        std::fputs("reactor request close win pending not cleared\n", stderr);
        return 16;
    }
    if (!client_driver.closed() || client_driver.last_error() != net::errc::closed) {
        std::fputs("reactor request close win driver state mismatch\n", stderr);
        return 17;
    }

    std::puts("net reactor request close win smoke: ok");
    return 0;
}
