...
function Validate-LogFile {
    ...
    Write-Host "Resident ELF platform smoke log validation"
...
function Get-SyntheticPassingLog {
    ...
    [void]$Text.AppendLine("Resident ELF platform smoke passed.")
...
function Invoke-SelfTest {
    ...
    Write-Host "Resident ELF platform smoke self-test passed."
    return 0
}
...
if ([string]::IsNullOrWhiteSpace($Log)) {
    $Log = Join-Path $ProjectRoot "cmake-build-h747-lab-debug\h747_lab_dev_loader_usb_cdc_elf_platform_smoke.log"
}
$ResolvedLog = [System.IO.Path]::GetFullPath($Log)

$AppRunSmoke = Join-Path $PSScriptRoot "capture-dev-loader-usb-cdc-app-run-smoke.ps1"
$TransferSmoke = Join-Path $PSScriptRoot "capture-dev-loader-usb-cdc-appstore-transfer-smoke.ps1"
foreach ($Script in @($AppRunSmoke, $TransferSmoke)) {
...
if ($DryRun) {
    Write-Host "Resident ELF platform smoke dry run"
...
try {
    ...
    Write-CaptureText "Resident ELF platform smoke`n"
    ...
    if (-not [string]::IsNullOrWhiteSpace($UsbPort)) {
        $ReceivedArgs += @("-UsbPort", $UsbPort)
    }
    & powershell @ReceivedArgs
    ...
    foreach ($Name in $MediaList) {
        $TransferLog = [System.IO.Path]::ChangeExtension($ResolvedLog, ".$Name-transfer.log")
        $TransferArgs = @(
            "-NoProfile",
            "-ExecutionPolicy", "Bypass",
            "-File", $TransferSmoke,
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "store_transfer_failed: $Name transfer exited with $LASTEXITCODE"
        }
        Merge-LogFile -Path $TransferLog -Header "$Name store transfer log"
    ...
    if ($Missing.Count -ne 0) {
        Write-CaptureText "`nResident ELF platform smoke failed. Missing tokens:`n"
        foreach ($Token in $Missing) {
    ...
    Write-CaptureText "`nResident ELF platform smoke passed.`n"
    exit 0
} finally {
    if ($null -ne $script:LogWriter) {
