...
try {
    Invoke-LoggedExternalTool `
        ...
        -FailureMessage "net lab route precise delete configure failed" `
        -Phase "configure"
    ...
    Invoke-LoggedExternalTool `
        ...
        -FailureMessage "net lab route precise delete build failed" `
        -Phase "build"
    ...
    $runOutput = Invoke-LoggedProgram `
        -Executable $exePath `
        -LogPath $runLogPath `
        -FailureMessage "net lab route precise delete run failed"
} finally {
    Pop-Location
}

$expectedLine = "net lab route precise delete smoke: ok"
if (($runOutput -join "`n") -notmatch [regex]::Escape($expectedLine)) {
    throw "net lab route precise delete smoke did not report success"
}

$checkLines = @(
    ("summary: {0}" -f $checkTextPath),
    "profile: net-lab-route-precise-delete-smoke",
