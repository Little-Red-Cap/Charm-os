...
namespace {
    ...
    constexpr auto kProductStr = usb::make_ascii_string_descriptor("Charm MSC+CDC Mock");
    ...
    std::vector<usb::u8> drain_in_transaction(usb::mock::Session& session, usb::u8 ep) {
        std::vector<usb::u8> out{};
        while (auto pkt = session.poll_in()) {
            if (pkt->ep != ep) {
                return {};
...
int main() {
    ...
    if (!expect(binding.cdc.has_value(), "cdc instance missing")) return 1;
    ...
    if (!expect(dev_desc[4] == 0xEF && dev_desc[5] == 0x02 && dev_desc[6] == 0x01, "composite device class mismatch")) return 1;

    if (!control_out(session, usb::SetupPacket{0x00, 0x05, 0x0009, 0x0000, 0x0000})) {
        std::fprintf(stderr, "[ERR] set address failed\n");
        return 1;
    }
    if (!expect(session.address() == 9, "device address not applied")) return 1;
    ...
    const auto total_length = static_cast<usb::u16>(cfg9[2] | (cfg9[3] << 8));
    if (!expect(total_length > 64, "composite config descriptor should span multiple packets")) return 1;

    const auto cfg_full = control_in(session, usb::SetupPacket{0x80, 0x06, 0x0200, 0x0000, 0x00FF});
    if (!expect(cfg_full.size() == total_length, "full config descriptor length mismatch")) return 1;
    if (!expect(cfg_full[4] == 3, "composite interface count mismatch")) return 1;
    if (!expect(cfg_full[9] == 0x08 && cfg_full[10] == 0x0B, "IAD descriptor missing")) return 1;
    ...
    if (!expect(session.endpoint_state(msc_cfg.ep_out).opened, "msc bulk out endpoint not opened")) return 1;
    if (!expect(session.endpoint_state(msc_cfg.ep_in).opened, "msc bulk in endpoint not opened")) return 1;
    if (!expect(session.endpoint_state(cdc_cfg.ep_notify).opened, "cdc notify endpoint not opened")) return 1;
    if (!expect(session.endpoint_state(cdc_cfg.ep_out).opened, "cdc bulk out endpoint not opened")) return 1;
    if (!expect(session.endpoint_state(cdc_cfg.ep_in).opened, "cdc bulk in endpoint not opened")) return 1;

    const auto max_lun = control_in(session, usb::SetupPacket{0xA1, 0xFE, 0x0000, msc_cfg.interface_number, 0x0001});
    if (!expect(max_lun.size() == 1 && max_lun[0] == 0x00, "max lun response mismatch")) return 1;

    const auto line = control_in(session, usb::SetupPacket{0xA1, 0x21, 0x0000, cdc_cfg.ctrl_ifc, 0x0007});
    ...
    const auto new_line_bytes = std::span<const usb::u8>(
        ...
        sizeof(new_line));
    if (!control_out(session, usb::SetupPacket{0x21, 0x20, 0x0000, cdc_cfg.ctrl_ifc, 0x0007}, new_line_bytes)) {
        std::fprintf(stderr, "[ERR] set line coding failed\n");
        return 1;
    }
    if (!expect(binding.cdc->line_coding().baud == 921600, "cdc line coding callback mismatch")) return 1;

    if (!control_out(session, usb::SetupPacket{0x21, 0x22, 0x0003, cdc_cfg.ctrl_ifc, 0x0000})) {
        std::fprintf(stderr, "[ERR] set control line state failed\n");
        return 1;
    }
    if (!expect(binding.cdc->control_line_state() == 0x0003, "cdc control line state mismatch")) return 1;

    constexpr char kPing[] = "ping";
    if (!expect(session.feed_out(cdc_cfg.ep_out,
                                 std::span<const usb::u8>(reinterpret_cast<const usb::u8*>(kPing), 4)),
                "cdc bulk OUT feed failed")) return 1;
    if (!expect(binding.cdc_rx_len == 4, "cdc bulk out length mismatch")) return 1;
    if (!expect(std::memcmp(binding.cdc_rx_buf.data(), kPing, 4) == 0, "cdc bulk out payload mismatch")) return 1;

    constexpr char kPong[] = "pong";
    std::memcpy(binding.cdc_tx_buf.data(), kPong, 4);
    binding.cdc_tx_len = 4;
    if (!expect(usb::device::examples::send_cdc_in_packet(session.dcd_ops(), &session, *binding.cdc, cdc_cfg.ep_mps),
                "cdc bulk IN send failed")) return 1;
    auto cdc_in = session.poll_in();
    if (!expect(cdc_in.has_value(), "cdc bulk IN packet missing")) return 1;
    if (!expect(cdc_in->ep == cdc_cfg.ep_in, "cdc bulk IN endpoint mismatch")) return 1;
    if (!expect(cdc_in->data.size() == 4 && std::memcmp(cdc_in->data.data(), kPong, 4) == 0, "cdc bulk IN payload mismatch")) return 1;
    if (!expect(session.ack_in(cdc_in->ep, cdc_in->data.size(), cdc_in->zlp), "cdc bulk IN ack failed")) return 1;
    if (!expect(binding.cdc_tx_len == 0, "cdc tx length not drained")) return 1;

    if (!expect(usb::device::examples::send_cdc_serial_state(session.dcd_ops(), &session, *binding.cdc, 0x0003),
                "cdc notify send failed")) return 1;
    const auto notify = drain_in_transaction(session, cdc_cfg.ep_notify);
    if (!expect(notify.size() == 10, "cdc notify length mismatch")) return 1;
    if (!expect(notify[8] == 0x03 && notify[9] == 0x00, "cdc notify payload mismatch")) return 1;

    std::vector<usb::u8> resp{};
    if (!run_scsi_in(session, *demo.bot, msc_cfg, usb::class_driver::ScsiCmd::inquiry, 36, 1, resp)) {
    ...
    if (!run_scsi_in(session, *demo.bot, msc_cfg, usb::class_driver::ScsiCmd::read_capacity_10, 8, 2, resp)) {
    ...
    if (!run_scsi_in(session, *demo.bot, msc_cfg, usb::class_driver::ScsiCmd::read_10, 512, 3, resp, 0, 1)) {
    ...
    std::printf("[OK] usb-msc-cdc-mock-smoke passed\n");
