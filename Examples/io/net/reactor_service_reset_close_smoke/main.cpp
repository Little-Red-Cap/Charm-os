...
int main() {
    ...
    if (!winsock.ok) {
        std::fputs("reactor service reset close winsock startup failed\n", stderr);
        return 1;
    }
    ...
    util::u16 port = 0;
    for (util::u16 candidate = 32620; candidate < 32720; ++candidate) {
    ...
    if (port == 0) {
        std::fputs("reactor service reset close listen failed\n", stderr);
        return 2;
    }

    NativeSocket client{::socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)};
    if (!client.valid()) {
        std::fputs("reactor service reset close native socket failed\n", stderr);
        return 3;
    }
    ...
    if (::connect(client.value,
        ...
        std::fputs("reactor service reset close native connect failed\n", stderr);
        return 4;
    }
    ...
    if (!accepted || accept_error != net::errc::ok) {
        std::fputs("reactor service reset close accept failed\n", stderr);
        return 5;
    }
    ...
    auto route = server_session.set_deferred_route(0x72u,
                                                   ...
                                                   &server_state);
    if (!route) {
        std::fputs("reactor service reset close route registration failed\n", stderr);
        return 6;
    }

    Driver server_driver{reactor, poller, server_binding, server_session};
    if (!server_driver.start()) {
        std::fputs("reactor service reset close driver start failed\n", stderr);
        return 7;
    }
    ...
    if (!send_all(client.value, wire, sizeof(wire))) {
        std::fputs("reactor service reset close native send failed\n", stderr);
        return 8;
    }
    ...
    if (!routed) {
        std::fputs("reactor service reset close route timeout\n", stderr);
        return 9;
    }

    if (!set_abortive_close(client.value)) {
        std::fputs("reactor service reset close linger setup failed\n", stderr);
        return 10;
    }
    client.close();
    ...
    if (!done) {
        std::fputs("reactor service reset close timeout\n", stderr);
        return 11;
    }
    if (!server_state.late_reply_rejected) {
        std::fputs("reactor service reset close late reply not rejected\n", stderr);
        return 12;
    }
    if (!server_driver.closed() || server_driver.last_error() != net::errc::closed) {
        std::fputs("reactor service reset close driver state mismatch\n", stderr);
        return 13;
    }
    if (server_session.has_deferred() || server_session.deferred_count() != 0) {
        std::fputs("reactor service reset close deferred not cleared\n", stderr);
        return 14;
    }
    if (server_session.last_error() != net::errc::closed) {
        std::fputs("reactor service reset close session error mismatch\n", stderr);
        return 15;
    }

    std::puts("net reactor service reset close smoke: ok");
    return 0;
}
