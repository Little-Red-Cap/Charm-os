param(
    ...
    [string]$OutputRoot = "out/net-lab-forward-explain-smoke",
    [int]$Jobs = 0,
    [switch]$Fresh
)
...
$targetName = "net-lab-forward-explain-smoke"
$exampleRoot = Resolve-FullPath -Path (Join-Path $repoRoot "Examples\io\net\net_lab_forward_explain_smoke")
...
try {
    Invoke-LoggedExternalTool `
        ...
        -FailureMessage "net lab forward explain configure failed" `
        -Phase "configure"
    ...
    Invoke-LoggedExternalTool `
        ...
        -FailureMessage "net lab forward explain build failed" `
        -Phase "build"
    ...
    $runOutput = Invoke-LoggedProgram `
        -Executable $exePath `
        -LogPath $runLogPath `
        -FailureMessage "net lab forward explain run failed"
} finally {
    Pop-Location
}

$expectedLine = "net lab forward explain smoke: ok"
if (($runOutput -join "`n") -notmatch [regex]::Escape($expectedLine)) {
    throw "net lab forward explain smoke did not report success"
}

$checkLines = @(
    ("summary: {0}" -f $checkTextPath),
    "profile: net-lab-forward-explain-smoke",
