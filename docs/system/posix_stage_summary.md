...
- when no live terminal fd remains in the table, `open("/dev/console", ...)` and `open("/dev/tty", ...)` fail with `ENOENT`
...
- `/dev/console` and `/dev/tty` now also have a pinned no-live-term boundary: once stdio has been fully rebound away from terminal-backed descriptors, both aliases fail with `ENOENT` instead of silently aliasing a non-terminal fd
