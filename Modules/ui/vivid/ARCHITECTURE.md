...
- 定位：面向资源相对充裕平台的“富 UI”实现，强调可组合控件与可维护渲染管线。
- 目标：零动态分配、可预测性能、可逐步演进的 UI 系统。
...
Modules/ui/vivid/
  ...
  font/      # 字体与排版：font/typography/生成器/lvgl 转换
  widgets/   # 具体控件集合
...
- `font_builder.py`：位图字体生成工具（默认 4bpp）
- `font_12.cppm`：示例 1bpp 字体
- `lvgl/`：LVGL 字体转换与样本（4bpp）

### 2.4 widgets

控件集合：button/slider/checkbox/list/scroll_container/text_area 等。

## 3. 核心数据结构

### 3.1 WidgetHandle

- 句柄由 `(kind, index, generation)` 组成。
...
- Style 是控件样式容器（bg/border/font 等）。
- Theme 以模板槽保存每个控件的 Style。

## 4. 渲染管线

1. Gui::render 触发树遍历与布局应用
2. Layout：优先 flex，否则 anchor
