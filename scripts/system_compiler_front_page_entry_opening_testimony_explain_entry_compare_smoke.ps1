param(
    ...
    [string]$OpeningTestimonyExplainEntryRouteCompareRoot = "cmake-build-system-compiler-front-page-entry-opening-testimony-explain-entry-route-compare-smoke",
    [string]$OutputRoot = "cmake-build-system-compiler-front-page-entry-opening-testimony-explain-entry-compare-smoke",
    [string]$PythonExe = "",
    [switch]$Clean
)
...
$openingTestimonyExplainEntryRouteCompareRootPath = Resolve-FullPath -Path $OpeningTestimonyExplainEntryRouteCompareRoot
...
$explainEntryRouteCompareSmokeScript = Join-Path $PSScriptRoot "system_compiler_front_page_entry_opening_testimony_explain_entry_route_compare_smoke.ps1"
...
try {
    ...
    if (
        ...
        Write-Host "[FRONT-PAGE-ENTRY-OPENING-TESTIMONY-EXPLAIN-ENTRY-COMPARE-SMOKE] explain_entry_bootstrap=reuse-existing"
    } else {
        Invoke-ExternalTool `
    ...
    $standingRouteCompareExplainEntryPath = Join-Path $openingTestimonyExplainEntryRouteCompareRootPath "standing-route-compare-explain-entry\front-page.entry-opening-testimony.explain-entry.summary.json"
    if ((-not $Clean) -and (Test-Path -LiteralPath $standingRouteCompareExplainEntryPath)) {
        Write-Host "[FRONT-PAGE-ENTRY-OPENING-TESTIMONY-EXPLAIN-ENTRY-COMPARE-SMOKE] route_compare_explain_entry_bootstrap=reuse-existing"
    } else {
        Invoke-ExternalTool `
            -Executable $powerShellExe `
            -ArgumentList @(
                ...
                "-OutputRoot",
                $openingTestimonyExplainEntryRouteCompareRootPath,
                "-PythonExe",
                $resolvedPythonExe,
                "-Clean"
            ) `
            -FailureMessage "opening testimony route-compare explain-entry smoke bootstrap failed"
    }
    ...
    foreach ($case in $cases) {
        ...
        Invoke-ExternalTool `
            ...
            -FailureMessage ("opening testimony explain-entry compare export failed for case '{0}'" -f $case.Name)
        ...
        Invoke-ExternalTool `
            ...
            -FailureMessage ("opening testimony explain-entry compare validation failed for case '{0}'" -f $case.Name)
        ...
        Write-Host (
            "[FRONT-PAGE-ENTRY-OPENING-TESTIMONY-EXPLAIN-ENTRY-COMPARE-SMOKE] case={0} verdict={1} candidate_status={2} selected_changed={3}" -f
            $case.Name,
...
Write-Host ("[FRONT-PAGE-ENTRY-OPENING-TESTIMONY-EXPLAIN-ENTRY-COMPARE-SMOKE] output_root={0}" -f $outputRootPath)
