...
#   charm_example_link_charm(<target_name> <charm_root> LINK_TARGET <target>)
...
function(charm_example_link_charm target_name charm_root)
    set(options)
    set(oneValueArgs LINK_TARGET)
    set(multiValueArgs CONFIG_TARGETS)
    cmake_parse_arguments(CHARM_EXAMPLE "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})

    if (NOT CHARM_EXAMPLE_LINK_TARGET)
        set(CHARM_EXAMPLE_LINK_TARGET Charm-os)
    endif()
    if (CHARM_EXAMPLE_UNPARSED_ARGUMENTS)
        list(APPEND CHARM_EXAMPLE_CONFIG_TARGETS ${CHARM_EXAMPLE_UNPARSED_ARGUMENTS})
    endif()

    add_subdirectory("${charm_root}" "${CMAKE_BINARY_DIR}/Charm")
    target_link_libraries(${target_name} PRIVATE ${CHARM_EXAMPLE_LINK_TARGET})
    if (CHARM_EXAMPLE_CONFIG_TARGETS)
        include("${charm_root}/cmake/CharmTargetConfig.cmake")
        charm_apply_config_targets(Charm-build-config ${CHARM_EXAMPLE_CONFIG_TARGETS})
        charm_apply_config_targets(${target_name} ${CHARM_EXAMPLE_CONFIG_TARGETS})
    endif()
endfunction()
