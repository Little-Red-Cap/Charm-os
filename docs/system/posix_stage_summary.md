...
- the exported C fs header surface now also smoke-pins relative cleanup directly: `unlink("parent-renamed.txt")` and `rmdir("sub")` both resolve against the live cwd and leave the removed relative paths reporting `ENOENT`
