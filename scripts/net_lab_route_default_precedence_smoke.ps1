param(
    ...
    [string]$OutputRoot = "out/net-lab-route-default-precedence-smoke",
    [int]$Jobs = 0,
    [switch]$Fresh
)
...
$targetName = "net-lab-route-default-precedence-smoke"
$exampleRoot = Join-Path $repoRoot "Examples\io\net\net_lab_route_default_precedence_smoke"
$buildDir = Join-Path $repoRoot "cmake-build-verify-net-lab-route-default-precedence-smoke"
...
try {
    Invoke-LoggedExternalTool `
        ...
        -FailureMessage "net lab route default precedence configure failed" `
        -Phase "configure"
    ...
    Invoke-LoggedExternalTool `
        ...
        -FailureMessage "net lab route default precedence build failed" `
        -Phase "build"
    ...
    $runOutput = Invoke-LoggedProgram `
        -Executable $exePath `
        -LogPath $runLogPath `
        -FailureMessage "net lab route default precedence run failed"
} finally {
    Pop-Location
}

$expectedLine = "net lab route default precedence smoke: ok"
if (($runOutput -join "`n") -notmatch [regex]::Escape($expectedLine)) {
    throw "net lab route default precedence smoke did not report success"
}

$checkLines = @(
    ("summary: {0}" -f $checkTextPath),
    "profile: net-lab-route-default-precedence-smoke",
