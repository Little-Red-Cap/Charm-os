param(
    ...
    [string]$OutputRoot = "out/net-lab-route-delete-smoke",
    [int]$Jobs = 0,
    [switch]$Fresh
)
...
$targetName = "net-lab-route-delete-smoke"
$exampleRoot = Join-Path $repoRoot "Examples\io\net\net_lab_route_delete_smoke"
$buildDir = Join-Path $repoRoot "cmake-build-verify-net-lab-route-delete-smoke"
...
try {
    Invoke-LoggedExternalTool `
        ...
        -FailureMessage "net lab route delete configure failed" `
        -Phase "configure"
    ...
    Invoke-LoggedExternalTool `
        ...
        -FailureMessage "net lab route delete build failed" `
        -Phase "build"
    ...
    $runOutput = Invoke-LoggedProgram `
        -Executable $exePath `
        -LogPath $runLogPath `
        -FailureMessage "net lab route delete run failed"
} finally {
    Pop-Location
}

$expectedLine = "net lab route delete smoke: ok"
if (($runOutput -join "`n") -notmatch [regex]::Escape($expectedLine)) {
    throw "net lab route delete smoke did not report success"
}

$checkLines = @(
    ("summary: {0}" -f $checkTextPath),
    "profile: net-lab-route-delete-smoke",
