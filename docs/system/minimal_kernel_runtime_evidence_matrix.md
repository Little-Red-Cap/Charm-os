...
| `TaskMessageDispatch` | `kernel.task_message_dispatch` | `Examples/kernel/runtime_task_message_dispatch_host` | `TaskMessageApi + TaskMessageTable` 的 `receive -> dispatch -> reply` bridge，direct dispatch、单次 `serve_once()`、handled/replied 分离，以及 missing label / reply failure 的 host 证据 | 不证明 long-running service loop、wait policy、service discovery、user ABI | 接 future service task loop |
