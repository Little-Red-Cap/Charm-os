...
try {
    ...
    foreach ($case in $cases) {
        ...
        Assert-Condition `
            ...
            -Message ("case '{0}' open action must expose projection_summary_lines" -f $case.Name)
        Assert-Condition `
            -Condition (@($actionSummary.open_action.projection_question_lines).Count -gt 0) `
            -Message ("case '{0}' open action must expose projection_question_lines" -f $case.Name)
        Assert-Condition `
        ...
        Assert-Condition `
            ...
            -Message ("case '{0}' opening preview summary lines must match open action projection summary lines" -f $case.Name)
        Assert-Condition `
            ...
            -Message ("case '{0}' opening preview question lines must match open action projection question lines" -f $case.Name)

        Write-Host (
            "[FRONT-PAGE-ENTRY-OPENING-FLOW-CONSUMER-PLAN-ACTION-SMOKE] case={0} selector={1} action={2} kind={3} entry={4} query={5}/{6} reason={7} projection_summary={8} projection_questions={9}" -f
            $case.Name,
