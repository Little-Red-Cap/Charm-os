...
ARMv7-A handoff launch, target=0x40200000, arg0=0x4023...., mode=sys, state=arm, transfer=yes, hook=yes, probe=yes, arg0=yes, stack=yes, branch=yes, return=yes, invoke=yes, launch=yes
```
...
- A third handoff-side `handoff launch` line now turns that pre-branch seam
  ...
  capture, branch-state, and return-state all still match the transfer
  contract. This keeps the seam honest without pretending we already jump into
  the real next image.
