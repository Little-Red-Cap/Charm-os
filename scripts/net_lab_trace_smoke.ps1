param(
    ...
    [string]$OutputRoot = "out/net-lab-trace-smoke",
    [int]$Jobs = 0,
    [switch]$Fresh
)
...
$targetName = "net-lab-trace-smoke"
...
try {
    Invoke-LoggedExternalTool `
        ...
        -FailureMessage "net lab trace configure failed" `
        -Phase "configure"
    ...
    Invoke-LoggedExternalTool `
        ...
        -FailureMessage "net lab trace build failed" `
        -Phase "build"
    ...
    $runOutput = Invoke-LoggedProgram `
        -Executable $exePath `
        -LogPath $runLogPath `
        -FailureMessage "net lab trace run failed"
} finally {
    Pop-Location
}

$expectedLine = "net lab trace smoke: ok"
if (($runOutput -join "`n") -notmatch [regex]::Escape($expectedLine)) {
    throw "net lab trace smoke did not report success"
}

$checkLines = @(
    ("summary: {0}" -f $checkTextPath),
    "profile: net-lab-trace-smoke",
