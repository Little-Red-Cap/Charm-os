...
- `Examples/ui/vivid/evidence_lab_manifest_demo`: verifies Evidence Lab Manifest v0, including stable run/tag/cases registry shape, unique stdout tags, axis coverage, Intent-to-Artifact anchor, vocabulary anchor, stdout law sync, CMake gate sync, and promotion boundary; stdout follows [`vivid_evidence_stdout_law.md`](vivid_evidence_stdout_law.md), guarded by CTest final `result=ok cases=8`.
