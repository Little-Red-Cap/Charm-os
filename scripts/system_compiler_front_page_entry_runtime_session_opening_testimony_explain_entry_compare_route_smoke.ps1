param(
    [string]$RuntimeSessionExplainEntryCompareRoot = "cmake-build-system-compiler-front-page-entry-runtime-session-opening-testimony-explain-entry-compare-smoke",
    [string]$OutputRoot = "cmake-build-system-compiler-front-page-entry-runtime-session-opening-testimony-explain-entry-compare-route-smoke",
    [string]$PythonExe = "",
    [switch]$Clean
)
...
$runtimeSessionExplainEntryCompareRootPath = Resolve-FullPath -Path $RuntimeSessionExplainEntryCompareRoot
...
$compareSmokeScript = Join-Path $PSScriptRoot "system_compiler_front_page_entry_runtime_session_opening_testimony_explain_entry_compare_smoke.ps1"
...
try {
    $standingComparePath = Join-Path $runtimeSessionExplainEntryCompareRootPath "self-standing\front-page.entry-opening-testimony.explain-entry.compare.summary.json"
    $driftedComparePath = Join-Path $runtimeSessionExplainEntryCompareRootPath "clean-to-landing-compare\front-page.entry-opening-testimony.explain-entry.compare.summary.json"
    $collapsedComparePath = Join-Path $runtimeSessionExplainEntryCompareRootPath "ready-to-blocked\front-page.entry-opening-testimony.explain-entry.compare.summary.json"
    if (
        (-not $Clean) -and
        ...
        Write-Host "[RUNTIME-SESSION-OPENING-TESTIMONY-EXPLAIN-ENTRY-COMPARE-ROUTE-SMOKE] compare_bootstrap=reuse-existing"
    } else {
        Invoke-ExternalTool `
            ...
            -ArgumentList @(
                ...
                $compareSmokeScript,
                "-OutputRoot",
                $runtimeSessionExplainEntryCompareRootPath,
                "-PythonExe",
                $resolvedPythonExe,
                "-Clean"
            ) `
            -FailureMessage "runtime session opening testimony explain-entry compare smoke bootstrap failed"
    }
    ...
    foreach ($case in $routeCases) {
        ...
        Invoke-ExternalTool `
            ...
            -FailureMessage ("runtime session opening testimony explain-entry compare route export failed for case '{0}'" -f $case.Name)
        ...
        Invoke-ExternalTool `
            ...
            -FailureMessage ("runtime session opening testimony explain-entry compare route validation failed for case '{0}'" -f $case.Name)
        ...
        Assert-Condition `
            -Condition ([string]$routeSummary.root_surface.summary_path -eq (Resolve-FullPath -Path $case.SourceSummary)) `
            -Message ("case '{0}' expected root surface path to remain the compare summary" -f $case.Name)
        $level1SurfaceIds = @(
        ...
        $serializedRoute = $routeSummary | ConvertTo-Json -Depth 100 -Compress
        foreach ($forbiddenText in @("runtime_session_summary", "world_compare_summary")) {
            Assert-Condition `
                -Condition (-not $serializedRoute.Contains($forbiddenText)) `
                -Message ("case '{0}' should not contain forbidden raw evidence field '{1}'" -f $case.Name, $forbiddenText)
        }

        Write-Host (
            "[RUNTIME-SESSION-OPENING-TESTIMONY-EXPLAIN-ENTRY-COMPARE-ROUTE-SMOKE] route_case={0} entries={1} level1=[{2}]" -f
            $case.Name,
    ...
    foreach ($case in $routeCompareCases) {
        ...
        Invoke-ExternalTool `
            ...
            -FailureMessage ("runtime session opening testimony explain-entry compare route compare export failed for case '{0}'" -f $case.Name)
        ...
        Invoke-ExternalTool `
            ...
            -FailureMessage ("runtime session opening testimony explain-entry compare route compare validation failed for case '{0}'" -f $case.Name)
        ...
        Write-Host (
            "[RUNTIME-SESSION-OPENING-TESTIMONY-EXPLAIN-ENTRY-COMPARE-ROUTE-SMOKE] route_compare_case={0} verdict={1} changed_entries={2}" -f
            $case.Name,
    ...
    Invoke-ExternalTool `
        ...
        -FailureMessage "runtime session opening testimony explain-entry compare route explain-entry export failed"
    ...
    Invoke-ExternalTool `
        ...
        -FailureMessage "runtime session opening testimony explain-entry compare route explain-entry validation failed"
    ...
    Assert-Condition `
        ...
        -Message "expected runtime session explain-entry compare route explain-entry to be ready"
    Assert-Condition `
    ...
    Assert-Condition `
        -Condition ([string]$explainEntryFromCompareRoute.source_route_ref.summary_path -eq $driftedRoutePath) `
        -Message "expected explain-entry compare-route source to stay on the drifted compare-route summary"
    $serializedExplainEntry = $explainEntryFromCompareRoute | ConvertTo-Json -Depth 100 -Compress
    foreach ($forbiddenText in @("runtime_session_summary", "world_compare_summary")) {
        Assert-Condition `
            -Condition (-not $serializedExplainEntry.Contains($forbiddenText)) `
            -Message ("compare-route explain-entry should not contain forbidden raw evidence field '{0}'" -f $forbiddenText)
    }

    Write-Host (
        "[RUNTIME-SESSION-OPENING-TESTIMONY-EXPLAIN-ENTRY-COMPARE-ROUTE-SMOKE] explain_entry_from_compare_route status={0} selection={1} selected={2}" -f
...
Write-Host ("[RUNTIME-SESSION-OPENING-TESTIMONY-EXPLAIN-ENTRY-COMPARE-ROUTE-SMOKE] output_root={0}" -f $outputRootPath)
