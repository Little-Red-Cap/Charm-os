...
| `SystemCompilerSummary` | `system_compiler_summary`、`comparison.system_compiler_summary`、artifact_root 默认总览、`cases[*].formation_basis / binding_summary / bringup_summary` | root 级跨阶段总结果物已经出现，并开始携带单 case 成立 basis | 单个阶段摘要或单条 compare 统计 |
