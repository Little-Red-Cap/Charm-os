...
- Run the same evidence bundle and append the off-board QEMU ELF virtual-board smoke:
  ...
  `qemu_elf_domain`, `qemu_elf_backend`, `qemu_elf_display`,
  `qemu_elf_coverage`, and `qemu_elf_player_min_gui` tokens so the archived
  evidence shows the virtual runtime domain, capability contract, fixed display
  mode, coverage counts, and GUI timeline without opening the JSON manually.
