param(
    [string]$OpeningTestimonyLandingRouteCompareRoot = "cmake-build-system-compiler-front-page-entry-opening-testimony-landing-route-compare-smoke",
    ...
    [string]$OutputRoot = "cmake-build-system-compiler-front-page-entry-opening-testimony-explain-entry-route-compare-smoke",
    [string]$PythonExe = "",
    [switch]$Clean
)
...
$openingTestimonyLandingRouteCompareRootPath = Resolve-FullPath -Path $OpeningTestimonyLandingRouteCompareRoot
...
$routeCompareSmokeScript = Join-Path $PSScriptRoot "system_compiler_front_page_entry_opening_testimony_landing_route_compare_smoke.ps1"
...
try {
    $standingComparePath = Join-Path $openingTestimonyLandingRouteCompareRootPath "opening-testimony-route-self-standing\front-page.route.compare.summary.json"
    $improvedComparePath = Join-Path $openingTestimonyLandingRouteCompareRootPath "opening-testimony-route-clean-to-drift\front-page.route.compare.summary.json"
    $driftedComparePath = Join-Path $openingTestimonyLandingRouteCompareRootPath "opening-testimony-route-landing-to-compare\front-page.route.compare.summary.json"
    if (
        (-not $Clean) -and
        ...
        Write-Host "[FRONT-PAGE-ENTRY-OPENING-TESTIMONY-EXPLAIN-ENTRY-ROUTE-COMPARE-SMOKE] route_compare_bootstrap=reuse-existing"
    } else {
        Invoke-ExternalTool `
            ...
            -ArgumentList @(
                ...
                "-OpeningTestimonyLandingRouteRoot",
                $openingTestimonyLandingRouteRootPath,
                "-OutputRoot",
                $openingTestimonyLandingRouteCompareRootPath,
                "-PythonExe",
                $resolvedPythonExe,
                "-Clean"
            ) `
            -FailureMessage "opening testimony landing route compare smoke bootstrap failed"
    }
    ...
    $blockedLandingRoutePath = Join-Path $openingTestimonyLandingRouteRootPath "blocked-landing-route\front-page.route.summary.json"
    if (-not (Test-Path -LiteralPath $blockedLandingRoutePath)) {
        $blockedLandingPath = "cmake-build-system-compiler-front-page-entry-opening-testimony-landing-smoke\blocked-empty-explanation-landing\front-page.entry-opening-testimony.landing.summary.json"
        ...
        if (-not (Test-Path -LiteralPath $resolvedBlockedLandingPath)) {
            Invoke-ExternalTool `
                ...
                -FailureMessage "opening testimony landing route smoke bootstrap for collapsed fixture failed"
        }
        Invoke-ExternalTool `
            ...
            -ArgumentList @(
                ...
                "--output-root",
                (Join-Path $openingTestimonyLandingRouteRootPath "blocked-landing-route")
            ) `
            -FailureMessage "opening testimony blocked landing route export failed"
    }
    ...
    Invoke-ExternalTool `
        ...
        -FailureMessage "opening testimony collapsed route compare fixture export failed"
    ...
    foreach ($case in $cases) {
        ...
        Invoke-ExternalTool `
            ...
            -FailureMessage ("opening testimony route-compare explain-entry export failed for case '{0}'" -f $case.Name)
        ...
        Invoke-ExternalTool `
            ...
            -FailureMessage ("opening testimony route-compare explain-entry validation failed for case '{0}'" -f $case.Name)
        ...
        Write-Host (
            "[FRONT-PAGE-ENTRY-OPENING-TESTIMONY-EXPLAIN-ENTRY-ROUTE-COMPARE-SMOKE] case={0} status={1} selection={2} selected={3}" -f
            $case.Name,
...
Write-Host ("[FRONT-PAGE-ENTRY-OPENING-TESTIMONY-EXPLAIN-ENTRY-ROUTE-COMPARE-SMOKE] output_root={0}" -f $outputRootPath)
