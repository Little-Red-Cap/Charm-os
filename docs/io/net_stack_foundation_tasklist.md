...
- `net.ipv4` 已补上原始 IPv4 datagram 的通用 egress / TTL 重写 / 挂起队列，`net.forward` 已提供最小双口 IPv4 forwarding hop，并已从固定对穿推进到“最小静态路由语义”：支持 prefix route / optional next hop / route miss -> `ICMP destination unreachable` / proxy ARP 仅对可转发目标应答；`net_lab_forward_trace_smoke` 现可覆盖 `client -> router1 -> router2 -> destination` 的真实转发、TTL 递减、`ICMP time exceeded`、`destination unreachable` 与最终 `echo reply` 闭环
