...
namespace {
    ...
    [[nodiscard]] bool service_step(Client& client,
        ...
        if (!client_progress) {
            std::fprintf(stderr,
                         "udp diag forward route churn client service error=%d\n",
        ...
        if (!router1_progress) {
            std::fprintf(stderr,
                         "udp diag forward route churn router1 service error=%d\n",
        ...
        if (!router2_progress) {
            std::fprintf(stderr,
                         "udp diag forward route churn router2 service error=%d\n",
        ...
        if (!server_progress) {
            std::fprintf(stderr,
                         "udp diag forward route churn server service error=%d\n",
    ...
    template <typename Client>
    [[nodiscard]] bool drive_until_reply_window(Client& client,
                                                const net::diag::udp::EndpointServer<16>& server,
                                                util::usize forwarded_before,
                                                util::usize server_requests_before,
                                                util::usize server_replies_before,
                                                HostNode& client_node,
                                                ForwardHop& router1,
                                                ForwardHop& router2,
                                                HostNode& server_node,
                                                ...
                                                Link& middle_link,
                                                Link& server_link,
                                                util::usize max_steps = 32) noexcept {
        for (util::usize step = 0; step < max_steps; ++step) {
            ...
            const auto reply_pending = server_link.stats_b_to_a().pending;
            if (client.pending_count() == 1u
                && server.request_count() == (server_requests_before + 1u)
                && server.reply_count() == (server_replies_before + 1u)
                && router1.forwarded_count() == (forwarded_before + 1u)
                && router2.forwarded_count() == (forwarded_before + 1u)
                && reply_pending == 1u
                && server_link.pending_count() == 1u
                && client_link.idle()
                && middle_link.idle()) {
                return true;
            }
...
int main() {
    ...
    constexpr auto client_network = net::IpAddress::ipv4(10, 0, 0, 0);
    ...
    if (!client_init || !client_node.ready()) {
        return fail("net lab udp diag forward route churn smoke client init failed\n", 1);
    }
    ...
    if (!router1_a_init
        ...
        || !router1.ready()) {
        return fail("net lab udp diag forward route churn smoke router1 init failed\n", 2);
    }
    ...
    if (!router2_a_init
        ...
        || !router2.ready()) {
        return fail("net lab udp diag forward route churn smoke router2 init failed\n", 3);
    }
    ...
    if (!server_init || !server_node.ready()) {
        return fail("net lab udp diag forward route churn smoke server init failed\n", 4);
    }
    ...
    if (!bound_client
        ...
        return fail("net lab udp diag forward route churn smoke bind failed\n", 5);
    }
    ...
    if (!ping
        ...
        return fail("net lab udp diag forward route churn smoke prime ping send failed\n", 6);
    }
    ...
    if (!drive_until_settled(
            ...
            middle_link,
            server_link)) {
        return fail("net lab udp diag forward route churn smoke prime ping stalled\n", 7);
    }
    ...
    if (!client_target
        ...
        return fail("net lab udp diag forward route churn smoke prime ping failed\n", 8);
    }
    ...
    const auto forwarded_before_churn = router1.forwarded_count();
    const auto response_before_churn = client.response_count();
    const auto timeout_before_churn = client.timeout_count();
    const auto drop_before_churn = client.drop_count();
    const auto server_requests_before_churn = server.request_count();
    const auto server_replies_before_churn = server.reply_count();
    const auto server_queued_before_churn = server.queued_reply_count();
    ...
    if (!slow
        ...
        || client.queued_count() != 1u) {
        return fail("net lab udp diag forward route churn smoke slow send failed\n", 9);
    }
    client_state.slow_request_id = slow.value();

    if (!drive_until_reply_window(
            client,
            server,
            forwarded_before_churn,
            server_requests_before_churn,
            server_replies_before_churn,
            client_node,
            router1,
            router2,
            server_node,
            ...
            middle_link,
            server_link)) {
        return fail("net lab udp diag forward route churn smoke reply window stalled\n", 10);
    }

    if (!server_state.saw_slow_count
        || client_state.got_timeout
        || client_state.got_slow_count
        || client.timeout_count() != timeout_before_churn
        || client.drop_count() != drop_before_churn
        || router1.forwarded_count() != (forwarded_before_churn + 1u)
        || router2.forwarded_count() != (forwarded_before_churn + 1u)
        || server_link.stats_b_to_a().pending != 1u
        || server.request_count() != (server_requests_before_churn + 1u)
        || server.reply_count() != (server_replies_before_churn + 1u)
        || server.queued_reply_count() != server_queued_before_churn
        || client_state.failed
        || client_state.error_calls != 0u
        || server_state.error_calls != 0u) {
        return fail("net lab udp diag forward route churn smoke reply window mismatch\n", 11);
    }

    if (!router2.remove_route(client_network, 24u)
        || router2.route_count() != 0u
        || router2.remove_route(client_network, 24u)) {
        return fail("net lab udp diag forward route churn smoke delete route failed\n", 12);
    }
    ...
    if (!drive_until_settled(
            ...
            server_link,
            64)) {
        return fail("net lab udp diag forward route churn smoke churn settle stalled\n", 13);
    }

    if (!client_state.got_timeout
        || client_state.got_slow_count
        || client.pending_count() != 0u
        || client.response_count() != response_before_churn
        || client.timeout_count() != (timeout_before_churn + 1u)
        || client.drop_count() != drop_before_churn
        || client.last_error() != net::errc::ok
        || server.request_count() != (server_requests_before_churn + 1u)
        || server.reply_count() != (server_replies_before_churn + 1u)
        ...
        || router2.forwarded_count() != (forwarded_before_churn + 1u)
        || router1.destination_unreachable_count() != 0u
        || router2.destination_unreachable_count() != 1u
        || router1.ttl_expired_count() != 0u
        || router2.ttl_expired_count() != 0u
        || router1.proxy_arp_reply_count() != 1u
        || router2.proxy_arp_reply_count() != 1u
        ...
        || client_state.error_calls != 0u
        || server_state.error_calls != 0u) {
        return fail("net lab udp diag forward route churn smoke churn handling failed\n", 14);
    }

    auto restored_client_route = router2.set_gateway_route(
        client_network,
        24u,
        net::Ipv4ForwardingPort::a,
        router1_b_ip);
    if (!restored_client_route || router2.route_count() != 1u) {
        return fail("net lab udp diag forward route churn smoke restore route failed\n", 15);
    }
    ...
    const auto unreachable_before_recovery = router2.destination_unreachable_count();
    ...
    if (!count
        ...
        || client.queued_count() != 1u) {
        return fail("net lab udp diag forward route churn smoke recovery send failed\n", 16);
    }
    ...
    if (!drive_until_settled(
            ...
            middle_link,
            server_link)) {
        return fail("net lab udp diag forward route churn smoke recovery stalled\n", 17);
    }
    ...
    if (!client_state.got_count
        ...
        || router1.destination_unreachable_count() != 0u
        || router2.destination_unreachable_count() != unreachable_before_recovery
        || router1.ttl_expired_count() != 0u
        || router2.ttl_expired_count() != 0u
        || router1.proxy_arp_reply_count() != 1u
        || router2.proxy_arp_reply_count() != 1u
        ...
        || client_state.error_calls != 0u
        || server_state.error_calls != 0u) {
        return fail("net lab udp diag forward route churn smoke recovery failed\n", 18);
    }

    std::puts("net lab udp diag forward route churn smoke: ok");
    return 0;
}
