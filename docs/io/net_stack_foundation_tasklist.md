...
- `net.icmp` 已补上最小 echo codec/service/egress：`Stack -> Ipv4Service -> IcmpEchoService` ingress 可解析 echo request/reply，ARP 解析后可完成 ICMP echo request 发送，`net_icmp_smoke` 可回归 codec、unsupported drop、checksum 校验与 ARP 后发送成功这条主路径
