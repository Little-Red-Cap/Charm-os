...
foreach ($requiredPath in @($witnessRootSmokeScript, $exportScript, $validateScript)) {
    if (-not (Test-Path -LiteralPath $requiredPath)) { throw "missing path: $requiredPath" }
}

Push-Location $repoRoot
try {
    $cleanWitnessPath = Join-Path $runtimeSessionWitnessRootPath "default-no-compare-witness\front-page.entry-opening-flow.open-event.witness.summary.json"
    $driftWitnessPath = Join-Path $runtimeSessionWitnessRootPath "default-with-drift-compare-witness\front-page.entry-opening-flow.open-event.witness.summary.json"
    if ((-not $Clean) -and (Test-Path -LiteralPath $cleanWitnessPath) -and (Test-Path -LiteralPath $driftWitnessPath)) {
        Write-Host "[RUNTIME-SESSION-OPENING-TESTIMONY-LANDING-SMOKE] witness_bootstrap=reuse-existing"
    } else {
        Invoke-ExternalTool `
            ...
            -ArgumentList @(
                ...
                "Bypass",
                "-File",
                $witnessRootSmokeScript,
                "-RuntimeSessionOpenEventWitnessRoot",
                $runtimeSessionWitnessRootPath,
                "-OutputRoot",
                $runtimeSessionWitnessRootPath,
                "-PythonExe",
                $resolvedPythonExe,
            ...
            -FailureMessage "runtime session testimony witness-root smoke bootstrap failed"
    }
    ...
    foreach ($case in $cases) {
        ...
        Invoke-ExternalTool `
            ...
            -FailureMessage ("runtime session opening testimony landing export failed for case '{0}'" -f $case.Name)
        ...
        Invoke-ExternalTool `
            ...
            -FailureMessage ("runtime session opening testimony landing validation failed for case '{0}'" -f $case.Name)
        ...
        Assert-Condition `
            -Condition ([int]$summary.artifact_targets.evidence_ref_count -eq [int]$case.ExpectedEvidenceRefCount) `
            -Message ("case '{0}' expected evidence ref count '{1}' but got '{2}'" -f $case.Name, $case.ExpectedEvidenceRefCount, $summary.artifact_targets.evidence_ref_count)
        Assert-Condition `
        ...
        Write-Host (
            "[RUNTIME-SESSION-OPENING-TESTIMONY-LANDING-SMOKE] case={0} status={1} source_judgment={2} witness_status={3} evidence_refs={4} artifact_refs={5}" -f
            $case.Name,
...
Write-Host ("[RUNTIME-SESSION-OPENING-TESTIMONY-LANDING-SMOKE] output_root={0}" -f $outputRootPath)
