...
function Show-LogTail {
    ...
    Write-Output "[armv7a-qemu-runtime-thread] log tail:"
...
$outFile = Join-Path $PSScriptRoot "qemu-runtime-thread.log"
$errFile = Join-Path $PSScriptRoot "qemu-runtime-thread.err.log"
...
$phaseMarker = "ARMv7-A phase, stage=runtime-thread-port"
$nextPhaseMarker = "ARMv7-A phase, stage=runtime-live"
...
if (($threadLog -notmatch "ARMv7-A runtime thread port, yield-path=svc-call-frame, yield-ready=yes, sleep-path=svc-call-frame, sleep-ready=yes, trap-call=yes, thread-runtime=yes, port=yes, bridge=yes, thread=yes")) {
...
Write-Output "[ok] armv7a qemu runtime-thread smoke detected"
