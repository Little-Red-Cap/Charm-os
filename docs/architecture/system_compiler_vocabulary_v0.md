...
| `Fact` | `export case manifest.declared_facts` / `required_facts` / `audit_provided_facts`、`declared_contracts.requires`、artifact report `required_facts` / `provided_facts` / `fact_inventory` | 已有输入侧与报告侧载体；`fact_only` case 现在也能把 contract-local facts 送入 artifact report | 单纯等于 capability 名字 |
