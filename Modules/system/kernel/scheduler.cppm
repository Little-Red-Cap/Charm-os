...
export namespace kernel {
    ...
    public:
        ...
        [[nodiscard]] Snapshot snapshot_diff() noexcept {
            const auto current = snapshot();
            const auto prev = last_snapshot_;
            last_snapshot_ = current;

            Snapshot diff{};
            diff.stats.posted = current.stats.posted - prev.stats.posted;
            diff.stats.dropped = current.stats.dropped - prev.stats.dropped;
            diff.stats.dispatched = current.stats.dispatched - prev.stats.dispatched;
            diff.stats.filtered = current.stats.filtered - prev.stats.filtered;
            diff.stats.filtered_run = current.stats.filtered_run - prev.stats.filtered_run;
            diff.stats.budget_limited = current.stats.budget_limited - prev.stats.budget_limited;
            return diff;
        }
        ...
        struct TraceSnapshot {
            TraceRecord<typename Caps::TimeSource::Tick> record{};
            kernel::ssu::Meta ssu{};
        };
        ...
        [[nodiscard]] std::array<TraceSnapshot, Config::enable_trace ? Config::trace_capacity : 0> trace_snapshot() const noexcept {
            std::array<TraceSnapshot, Config::enable_trace ? Config::trace_capacity : 0> out{};
            if constexpr (Config::enable_trace) {
                ...
                for (std::size_t i = 0; i < trace_.size(); ++i) {
                    ...
                    out[i] = TraceSnapshot{
                        rec,
                        registry_->task_ssu_meta(rec.task),
                    };
                }
            }
            return out;
        ...
        [[nodiscard]] std::size_t trace_snapshot_size() const noexcept {
            if constexpr (Config::enable_trace) {
                return trace_.size();
            } else {
                return 0;
