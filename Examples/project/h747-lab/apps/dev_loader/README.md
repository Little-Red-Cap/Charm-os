...
- Run the same evidence bundle and append the off-board QEMU ELF virtual-board smoke:
  ...
  fixed display mode, frame/input/storage evidence counts, near-limit and
  over-limit ELF capacity, direct/received/packetstream/store source matrix
  coverage, coverage counts, and GUI timeline without opening the JSON manually.
