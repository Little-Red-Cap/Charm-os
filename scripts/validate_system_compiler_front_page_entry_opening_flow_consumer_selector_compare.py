...
from compare_system_compiler_front_page_entry_opening_flow_consumer_selector import build_compare_summary_model
from system_compiler_front_page_route_lib import load_json, normalize_path


COMPARE_SCHEMA_PATH = "schemas/system_compiler.front_page_entry_opening_flow_consumer_selector_compare.v0.schema.json"
...
def validate_references(summary: dict, errors: list[str]) -> None:
    ...
    for field in (
        "baseline_selector_summary_path",
        "candidate_selector_summary_path",
        "output_root",
    ...
    for index, provenance in enumerate(summary.get("selector_provenance", [])):
        if not isinstance(provenance, dict):
            errors.append(f"selector_provenance[{index}]: invalid provenance entry")
            continue
        ensure_exists(provenance.get("source_summary_path"), f"selector_provenance[{index}].source_summary_path", errors)
        ensure_exists(
            provenance.get("source_report_markdown_path"),
            f"selector_provenance[{index}].source_report_markdown_path",
            errors,
        )
        ensure_exists(
            provenance.get("source_check_text_path"),
            f"selector_provenance[{index}].source_check_text_path",
            errors,
        )

    for index, change in enumerate(summary.get("selector_entry_changes", [])):
        if not isinstance(change, dict):
            errors.append(f"selector_entry_changes[{index}]: invalid entry change")
            continue
        ensure_exists(
            change.get("baseline_target_summary_path"),
            f"selector_entry_changes[{index}].baseline_target_summary_path",
            errors,
            required=False,
        )
        ensure_exists(
            change.get("candidate_target_summary_path"),
            f"selector_entry_changes[{index}].candidate_target_summary_path",
            errors,
            required=False,
        )
        ensure_exists(
            change.get("baseline_opener_summary_path"),
            f"selector_entry_changes[{index}].baseline_opener_summary_path",
            errors,
            required=False,
        )
        ensure_exists(
            change.get("candidate_opener_summary_path"),
            f"selector_entry_changes[{index}].candidate_opener_summary_path",
            errors,
            required=False,
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate system compiler front_page entry opening-flow consumer selector compare summary."
    )
    parser.add_argument(
        "--summary",
        default="",
        help=(
            "Path to selector compare summary JSON. If omitted, "
            "--bundle-root/front-page.entry-opening-flow.consumer.selector.compare.summary.json is used."
        ),
    )
    parser.add_argument(
        "--bundle-root",
        default="",
        help="Bundle root containing front-page.entry-opening-flow.consumer.selector.compare.summary.json.",
    )
    ...
    if args.summary:
        ...
        bundle_root = Path(
            args.bundle_root or "out/system-compiler-front-page-entry-opening-flow-consumer-selector-compare"
        ).resolve()
        summary_path = bundle_root / "front-page.entry-opening-flow.consumer.selector.compare.summary.json"
    ...
    try:
        ...
        artifact_context = summary.get("artifact_context", {})
        baseline_selector_path = Path(artifact_context.get("baseline_selector_summary_path", "")).resolve()
        candidate_selector_path = Path(artifact_context.get("candidate_selector_summary_path", "")).resolve()
        ...
        jsonschema.validate(load_json(baseline_selector_path), selector_schema)
        jsonschema.validate(load_json(candidate_selector_path), selector_schema)

        expected_summary = build_compare_summary_model(
            baseline_selector_path=baseline_selector_path,
            candidate_selector_path=candidate_selector_path,
        ...
        for field in (
            ...
            "selector_status",
            "selector_changes",
            "selector_entry_summary",
            "selector_entry_changes",
            "selector_regression_surface",
            "questions",
            "violations",
        ):
    ...
    print(f"[OK] selector verdict -> {summary.get('selector_verdict', '')}")
    print(f"[OK] changed entries -> {summary.get('selector_entry_summary', {}).get('changed_entry_count', 0)}")
    return 0
