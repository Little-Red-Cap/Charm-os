...
public:
    ...
    MockDisplayWorld() : display_(framebuffer()) {}
...
bool write_ppm(const std::filesystem::path& path, host::world::MockDisplayWorld& world) {
...
std::filesystem::path output_path(const char* argv0) {
    if ((argv0 == nullptr) || (argv0[0] == '\0')) {
        return std::filesystem::current_path() / "display_raster_demo.ppm";
    }
    return std::filesystem::absolute(argv0).parent_path() / "display_raster_demo.ppm";
}
