param(
    ...
    [string]$OutputRoot = "cmake-build-system-compiler-front-page-entry-opening-testimony-explain-entry-smoke",
    [string]$PythonExe = "",
    [switch]$Clean
)
...
try {
    ...
    if (
        ...
        Write-Host "[FRONT-PAGE-ENTRY-OPENING-TESTIMONY-EXPLAIN-ENTRY-SMOKE] route_bootstrap=reuse-existing"
    } else {
        Invoke-ExternalTool `
            ...
            -FailureMessage "opening testimony landing route smoke bootstrap failed"
    }
    ...
    foreach ($case in $cases) {
        ...
        Invoke-ExternalTool `
            ...
            -FailureMessage ("opening testimony explain-entry export failed for case '{0}'" -f $case.Name)
        ...
        Invoke-ExternalTool `
            ...
            -FailureMessage ("opening testimony explain-entry validation failed for case '{0}'" -f $case.Name)
        ...
        Write-Host (
            "[FRONT-PAGE-ENTRY-OPENING-TESTIMONY-EXPLAIN-ENTRY-SMOKE] case={0} status={1} selection={2} selected={3}" -f
            $case.Name,
...
Write-Host ("[FRONT-PAGE-ENTRY-OPENING-TESTIMONY-EXPLAIN-ENTRY-SMOKE] output_root={0}" -f $outputRootPath)
