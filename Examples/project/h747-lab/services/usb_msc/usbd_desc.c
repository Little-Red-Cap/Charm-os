...
#define USBD_PID_FS_STORAGE 22337U
...
__ALIGN_BEGIN uint8_t USBD_FS_DeviceDesc[USB_LEN_DEV_DESC] __ALIGN_END = {
    ...
    LOBYTE(USBD_VID),
    HIBYTE(USBD_VID),
    LOBYTE(USBD_PID_FS_STORAGE),
    HIBYTE(USBD_PID_FS_STORAGE),
    0x00,
    0x01,
    USBD_IDX_MFC_STR,
...
uint8_t* USBD_FS_ProductStrDescriptor(USBD_SpeedTypeDef speed, uint16_t* length) {
    (void)speed;
    USBD_GetString((uint8_t*)"Charm H747 eMMC MSC", USBD_StrDesc, length);
    return USBD_StrDesc;
}
...
uint8_t* USBD_FS_ConfigStrDescriptor(USBD_SpeedTypeDef speed, uint16_t* length) {
    if (speed == USBD_SPEED_HIGH) {
        USBD_GetString((uint8_t*)"MSC Config HS", USBD_StrDesc, length);
        return USBD_StrDesc;
    }
    USBD_GetString((uint8_t*)"MSC Config FS", USBD_StrDesc, length);
    return USBD_StrDesc;
}
