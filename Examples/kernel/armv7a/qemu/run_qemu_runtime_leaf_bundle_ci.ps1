...
function Show-LogTail {
    ...
    Write-Output "[armv7a-qemu-runtime-leaf-bundle] log tail:"
...
$outFile = Join-Path $PSScriptRoot "qemu-runtime-leaf-bundle.log"
$errFile = Join-Path $PSScriptRoot "qemu-runtime-leaf-bundle.err.log"
...
$expected = @(
    ...
    "ARMv7-A phase, stage=runtime-package"
)
$missing = $expected | Where-Object { -not $log.Contains($_) }

$phaseMarker = "ARMv7-A phase, stage=runtime-leaf-bundle"
$nextPhaseMarker = "ARMv7-A phase, stage=runtime-package"
...
if (($bundleLog -notmatch "ARMv7-A runtime leaf bundle, tick-mode=oneshot, tick-route=irq, exception=yes, interrupt=yes, timer=yes, context=yes, current=yes, hook=yes, loop=yes, trap=yes, call=yes, thread=yes, live=yes, export=yes, ports=yes, bundle=yes")) {
...
Write-Output "[ok] armv7a qemu runtime-leaf-bundle smoke detected"
