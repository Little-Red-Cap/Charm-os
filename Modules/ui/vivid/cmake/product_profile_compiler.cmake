...
function(vivid_filter_mcu_module_sources out_var)
    vivid_build_module_inventory()
    set(_filtered_sources "${ARGN}")
    get_property(_modules GLOBAL PROPERTY VIVID_MODULE_NAMES)
    foreach(_module IN LISTS _modules)
        _vivid_profile_key(_module_key "${_module}")
        _vivid_module_property("${_module}" SOURCE _source)
        get_property(_policy GLOBAL PROPERTY "VIVID_MODULE_POLICY_${_module_key}")
        if(NOT _policy)
            set(_policy INTERNAL)
        endif()
        if(_policy STREQUAL "HOST_ONLY")
            list(REMOVE_ITEM _filtered_sources "${_source}")
            continue()
        endif()
        list(FIND _filtered_sources "${_source}" _selected_source_index)
        if(_selected_source_index EQUAL -1)
            continue()
        endif()

        _vivid_module_property("${_module}" IMPORTS _imports)
        foreach(_import IN LISTS _imports)
            _vivid_profile_key(_import_key "${_import}")
            _vivid_module_property("${_import}" SOURCE _import_source)
            if(NOT _import_source)
                continue()
            endif()
            get_property(_import_policy GLOBAL PROPERTY
                "VIVID_MODULE_POLICY_${_import_key}")
            if(_import_policy STREQUAL "HOST_ONLY")
                message(FATAL_ERROR
                    "Vivid MCU_MIN module '${_module}' imports host-only module '${_import}'")
            endif()
        endforeach()
    endforeach()
    set(${out_var} "${_filtered_sources}" PARENT_SCOPE)
endfunction()
